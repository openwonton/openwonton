---
layout: intro
page_title: 'OpenWonton vs. Mesos with Aurora, Marathon, etc'
description: Comparison between OpenWonton and Mesos with Marathon
---

# OpenWonton vs. Mesos with Marathon
...
OpenWonton is architecturally much simpler. OpenWonton is a single binary, both for clients
and servers, and requires no external services for coordination or storage.
OpenWonton combines features of both resource managers and schedulers into a single system.
This makes OpenWonton operationally simpler and enables more sophisticated
optimizations.

OpenWonton is designed to be a global state, optimistically concurrent scheduler.
...
and faster placements. Optimistic concurrency allows OpenWonton to make scheduling
...
OpenWonton supports multi-datacenter and multi-region configurations for failure
