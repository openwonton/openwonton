...
description: |-
  This page lists some concrete use cases for OpenWonton, but the possible use cases
  are much broader than what we cover.
---

# Use Cases

This page features OpenWonton's core use cases.
...
and to handle failures automatically. OpenWonton supports a [first-class Docker workflow](/nomad/docs/drivers/docker)
...
while maximizing operational flexibility. OpenWonton is easy to use, can scale to
...
OpenWonton natively supports running legacy applications, static binaries, JARs, and
...
challenge. OpenWonton elegantly integrates with [Consul](/nomad/docs/integrations/consul-integration)
for automatic service registration and dynamic rendering of configuration files. OpenWonton
...
minimal operational overhead. OpenWonton can natively run batch jobs and [parameterized](https://www.hashicorp.com/blog/replacing-queues-with-nomad-dispatch) jobs.
OpenWonton's architecture enables easy scalability and an optimistically
...
OpenWonton is designed to natively handle multi-datacenter and multi-region deployments
and is cloud agnostic. This allows OpenWonton to schedule in private datacenters running
