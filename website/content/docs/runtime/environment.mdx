...
description: Learn how to configure the OpenWonton runtime environment.
---
...
OpenWonton will pass both the allocation ID and name, the deployment ID that created
...
When you request resources for a job, OpenWonton creates a resource offer. The final
resources for your job are not determined until it is scheduled. OpenWonton will
tell you which resources have been allocated after evaluation and placement.

### CPU and Memory

OpenWonton will pass CPU and memory limits to your job as `NOMAD_CPU_LIMIT`,
`NOMAD_MEMORY_LIMIT`, and `NOMAD_MEMORY_MAX_LIMIT`. Your task should use these
values to adapt its behavior to fit inside the resource allocation that OpenWonton
...
OpenWonton assigns IP addresses and ports to your jobs and exposes them via environment
variables. See the [Networking](/nomad/docs/job-specification/network) page for more
details.

### Task Directories

OpenWonton creates a working directory for each allocation on a client. The
allocation working directory contains a task working directory for each task
in the allocation.

OpenWonton makes the following directories available to tasks, relative to the task
working directory:
...
environment variables. Prior to OpenWonton 0.5.5 the key was uppercased and since
...
OpenWonton passes the environment variables defined in the client host to tasks when
