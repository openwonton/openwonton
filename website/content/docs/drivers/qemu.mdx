...
The driver requires the image to be accessible from the OpenWonton client via the
...
- `driver.qemu` - Set to `1` if QEMU is found on the host node. OpenWonton determines
...
OpenWonton uses QEMU to provide full software virtualization for virtual machine
workloads. OpenWonton can use QEMU KVM's hardware-assisted virtualization to deliver
better performance.
