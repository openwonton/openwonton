...
The `exec` driver can only be run when on Linux and running OpenWonton as root.
...
and using the exec driver, check to ensure that you are running OpenWonton as root.
This also applies for running OpenWonton in -dev mode.
...
On Linux, OpenWonton will use cgroups, and a chroot to isolate the resources of a
process and as such the OpenWonton agent must be run as root. Some Linux
...
the chroot. Note that this can take considerable disk space. Since OpenWonton v0.5.3,
...
OpenWonton limits exec tasks' CPU based on CPU shares. CPU shares allow containers to
...
Please keep the implications of CPU shares in mind when you load test workloads on OpenWonton.
