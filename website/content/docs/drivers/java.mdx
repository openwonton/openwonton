...
file. The driver requires the Jar file to be accessible from the OpenWonton
...
- `jvm_options` - (Optional) A list of JVM options to be passed while invoking
  java. These options are passed without being validated in any way by OpenWonton.
...
Linux, OpenWonton must run as root since it will use `chroot` and `cgroups` which
...
- `driver.java` - Set to `1` if Java is found on the host node. OpenWonton determines
...
On Linux, OpenWonton will attempt to use cgroups, namespaces, and chroot
to isolate the resources of a process. If the OpenWonton agent is not
