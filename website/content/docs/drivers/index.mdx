...
description: Task Drivers are used to integrate with the host OS to run tasks in OpenWonton.
---

# Task Drivers

Task drivers are used by OpenWonton clients to execute a task and provide resource
isolation. By having extensible task drivers, OpenWonton has the flexibility to
support a broad set of workloads across all major operating systems.

Starting with OpenWonton 0.9, task drivers are now pluggable. This gives users the
flexibility to introduce their own drivers without having to recompile OpenWonton.
You can view the [plugin block][plugin] documentation for examples on how to
use the `plugin` block in OpenWonton's client configuration. Note that we have
...
built-in drivers are now plugins, OpenWonton remains a single binary and maintains
...
OpenWonton client CPU / memory / storage between tasks. Resource isolation
...
security-related controls, but the OpenWonton client to task interface should not be
considered a security boundary. See the [access control guide][acl_guide] for
more information on how to protect OpenWonton cluster operations.
