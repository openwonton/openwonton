...
The `docker` driver provides a first-class Docker workflow on OpenWonton. The Docker
...
- `image` - The Docker image to run. The image may include a tag or custom URL
  ...
  requires authentication credentials must be provided to OpenWonton. Please see the
...
- `image_pull_timeout` - (Optional) A time duration that controls how long OpenWonton
...
- `args` - (Optional) A list of arguments to the optional `command`. If no
  ...
  References to environment variables or any [interpretable OpenWonton
...
- `auth_soft_fail` `(bool: false)` - Don't fail the task on an auth failure.
  Attempt to continue without auth. If the OpenWonton client configuration has an
...
- `privileged` - (Optional) `true` or `false` (default). Privileged mode gives
  ...
  OpenWonton agent and docker daemon to be configured to allow privileged
  containers.
...
- `ipc_mode` - (Optional) The IPC mode to be used for the container. The default
  ...
  it is not possible to refer to Docker containers started by OpenWonton since their
  names are not known in advance. Note that setting this option also requires the
  OpenWonton agent to be configured to allow privileged containers.
...
- `load` - (Optional) Load an image from a `tar` archive file instead of from a
  ...
  ensure that OpenWonton keeps the archive intact after download.
...
- `network_mode` - (Optional) The network mode to be used for the container. In
  ...
  configuration on the host (which is outside the scope of OpenWonton). Valid values
...
- `pid_mode` - (Optional) `host` or not set (default). Set to `host` to share
  the PID namespace with the host. Note that this also requires the OpenWonton agent
...
- `SSL` - (Optional) If this is set to true, OpenWonton uses SSL to talk to the
...
- `uts_mode` - (Optional) `host` or not set (default). Set to `host` to share
  the UTS namespace with the host. Note that this also requires the OpenWonton agent
  to be configured to allow privileged containers.
...
OpenWonton creates a container after pulling an image. Containers are named
...
!> **Be Careful!** At this time these credentials are stored in OpenWonton in plain
...
interfaces, SDNs, etc. OpenWonton uses `bridged` networking by default, like Docker.
...
OpenWonton will automatically expose any ports specified in the `ports` field.
...
If OpenWonton allocates port `23332` to your api task for `http`, `23332` will be
...
If OpenWonton allocates port `23332` to your allocation, the Docker driver will
...
Up until OpenWonton 0.12, ports could be specified in a task's resource block and set using the docker
...
outside of OpenWonton. First-class support for these options may be improved later
through OpenWonton plugins or dynamic job configuration.
...
OpenWonton requires Docker to be installed and running on the host alongside the
OpenWonton agent. OpenWonton was developed against Docker `1.8.2` and `1.9`.

By default OpenWonton communicates with the Docker daemon using the daemon's Unix
socket. OpenWonton will need to be able to read/write to this socket. If you do not
run OpenWonton as root, make sure you add the OpenWonton user to the Docker group so
OpenWonton can communicate with the Docker daemon.

For example, on Ubuntu you can use the `usermod` command to add the `nomad`
user to the `docker` group so you can run OpenWonton without root:
...
on a OpenWonton client, you can modify them with the [plugin block][plugin-block]
...
- `endpoint` - If using a non-standard socket, HTTP or another location, or if
  TLS is being used, docker.endpoint must be set. If unset, OpenWonton will attempt
...
- `pull_activity_timeout` - Defaults to `2m`. If OpenWonton receives no communication
  from the Docker engine during an image pull within this timeframe, OpenWonton will
...
- `pids_limit` - Defaults to unlimited (`0`). An integer value that specifies
  the pid limit for all the Docker containers running on that OpenWonton client. You
...
- `disable_log_collection` - Defaults to `false`. Setting this to true will
  disable OpenWonton logs collection of Docker tasks. If you don't rely on nomad log
...
- `logging` block:
  ...
  - `type` - Defaults to `"json-file"`. Specifies the logging driver docker
    should use for all containers OpenWonton starts. Note that for older versions
    of Docker, only `json-file` file or `journald` will allow OpenWonton to read
...
- `gc` block:
  ...
  - `image` - Defaults to `true`. Changing this to `false` will prevent OpenWonton
  ...
  - `image_delay` - A time duration, as [defined
    ...
    The delay controls how long OpenWonton will wait between an image being unused
  ...
  - `container` - Defaults to `true`. This option can be used to disable OpenWonton
    from removing a container when the task exits. Under a name conflict,
    OpenWonton may still remove the dead container.
  ...
  - `dangling_containers` block for controlling dangling container detection
    ...
    - `period` - Defaults to `"5m"`. A time duration that controls interval
      between OpenWonton scans for dangling containers.
...
- `infra_image_pull_timeout` - A time duration that controls how long OpenWonton will
...
- `docker.endpoint` - If using a non-standard socket, HTTP or another location,
  or if TLS is being used, `docker.endpoint` must be set. If unset, OpenWonton will
...
- `docker.cleanup.image` Defaults to `true`. Changing this to `false` will
  prevent OpenWonton from removing images from stopped tasks.
...
- `docker.cleanup.image.delay` A time duration, as [defined
  ...
  delay controls how long OpenWonton will wait between an image being unused and
...
- `docker.cleanup.container`: Defaults to `true`. This option can be used to
  disable OpenWonton from removing a container when the task exits. Under a name
  conflict, OpenWonton may still remove the dead container.
...
`DOCKER_TLS_VERIFY`, and `DOCKER_CERT_PATH` to customize OpenWonton's behavior. If
`docker.endpoint` is set OpenWonton will **only** read client configuration from the
config file.
...
OpenWonton limits containers' CPU based on CPU shares. CPU shares allow containers
...
OpenWonton limits containers' memory usage based on total virtual memory. This means
that containers scheduled by OpenWonton cannot use swap. This is to ensure that a
...
OpenWonton's Docker integration does not currently provide QoS around network or
...
OpenWonton 0.10.2 introduces a detector and a reaper for dangling Docker containers,
containers that OpenWonton starts yet does not manage or track. Though rare, they
lead to unexpectedly running services, potentially with stale versions.

When Docker daemon becomes unavailable as OpenWonton starts a task, it is possible
for Docker to successfully start the container but return a 500 error code from
the API call. In such cases, OpenWonton retries and eventually aims to kill such
...
container that OpenWonton no longer manages.

The newly added reaper periodically scans for such containers. It only targets
containers with a `com.hashicorp.nomad.allocation_id` label, or match OpenWonton's
conventions for naming and bind-mounts (i.e. `/alloc`, `/secrets`, `local`).
Containers that don't match OpenWonton container patterns are left untouched.
...
[winissues]: https://github.com/openwonton/openwonton/issues?q=is%3Aopen+is%3Aissue+label%3Atheme%2Fdriver%2Fdocker+label%3Atheme%2Fplatform-windows
