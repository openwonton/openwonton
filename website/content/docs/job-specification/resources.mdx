...
This example specifies that the task requires 2 reserved cores. With this block, OpenWonton will find
a client with enough spare capacity to reserve 2 cores exclusively for the task. Unlike the `cpu` field, the
task will not share cpu time with any other tasks managed by OpenWonton on the client.
...
unexpected load spikes, OpenWonton allows job authors to set two separate memory
limits:

* `memory`: the reserve limit to represent the task’s typical memory usage —
  this number is used by the OpenWonton scheduler to reserve and place the task
...
pressure the running tasks to free up memory. If the contention persists, OpenWonton
...
Memory oversubscription is opt-in. OpenWonton operators can enable [Memory
...
* Set `oom_score_adj` for Linux host services that aren't managed by OpenWonton, e.g.
  Docker, logging services, and the OpenWonton agent itself. For Systemd services, you can use the [`OOMScoreAdj` field](https://github.com/openwonton/openwonton/blob/v1.0.0/dist/systemd/nomad.service#L25).
...
* Set the [client `reserved`](/nomad/docs/configuration/client#reserved) with enough
  memory for host services that aren't managed by OpenWonton as well as a buffer
...
[device]: /nomad/docs/job-specification/device 'OpenWonton device Job Specification'
