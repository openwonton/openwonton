...
description: |-
  Input variables are parameters for OpenWonton jobs.
  This page covers configuration syntax for variables.
---

# Input Variables

Input variables serve as parameters for a OpenWonton job, allowing aspects of the
...
"variables" or "OpenWonton variables" when it is clear from context what sort of
variable is being discussed. Other kinds of variables in OpenWonton include
_environment variables_ (set by the shell where OpenWonton runs) and _expression
...
if no value is set when calling the job or running OpenWonton. The `default`
...
as easy reminders for users of the job, and allow OpenWonton to return a helpful
...
If condition evaluates to false, OpenWonton will produce an error message that includes
...
As a fallback for the other ways of defining variables, OpenWonton searches the
environment of its own process for environment variables named `NOMAD_VAR_`
followed by the name of a declared variable.

This can be useful when running OpenWonton in automation, or when running a
sequence of OpenWonton commands in succession with the same variables. For example,
...
OpenWonton matches the variable name exactly as given in configuration, and so the
required environment variable name will usually have a mix of upper and lower
case letters as in the above example.

### Complex-typed Values

When variable values are provided in a variable definitions file, OpenWonton's
...
variables. For convenience, OpenWonton defaults to interpreting `-var` and
...
require a complex value (list, set, map, object, or tuple), OpenWonton will instead
...
OpenWonton loads variables in the following order, with later sources taking
...
OpenWonton uses the _last_ value it finds, overriding any previous values. Note
