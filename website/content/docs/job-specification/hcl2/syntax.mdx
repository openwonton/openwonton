...
details of HCL in order to use OpenWonton, and so this page summarizes the most
...
Argument names, block type names, and the names of most OpenWonton-specific
...
For complete identifier rules, OpenWonton implements
...
OpenWonton configuration files must always be UTF-8 encoded. While the
delimiters of the language are all ASCII characters, OpenWonton accepts
non-ASCII characters in identifiers, comments, and string values.

OpenWonton accepts configuration files with either Unix-style line endings
