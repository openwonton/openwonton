...
result format may change in future versions of OpenWonton, based on feedback.
...
This function maps
[OpenWonton language values](/nomad/docs/job-specification/hcl2/expressions#types-and-values)
to YAML tags in the following way:

| OpenWonton type    | YAML type            |
| ------------- | -------------------- |
| `string`      | `!!str`              |
| `number`      | `!!float` or `!!int` |
| `bool`        | `!!bool`             |
...
Because the YAML format cannot fully represent all of the OpenWonton language
types, passing the `yamlencode` result to `yamldecode` will not produce an
identical value, but the OpenWonton language automatic type conversion rules
