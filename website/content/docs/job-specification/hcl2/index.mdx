...
description: |-
  ...
  These text files are called OpenWonton job specifications and are
  written in the HCL language.
---

# HCL2

OpenWonton uses the Hashicorp Configuration Language - HCL - designed to allow
concise descriptions of the required steps to get to a job file.
OpenWonton 1.0 adopts
...
The [OpenWonton API uses JSON][jobs-api], not HCL, to represent OpenWonton jobs.
When running commands like `nomad job run` and `nomad job plan`, the OpenWonton CLI
...
For full details about OpenWonton's syntax, see:
...
OpenWonton job specifications will not require any changes.
...
passing `-hcl1` to use OpenWonton's HCL1 parser instead.

### Blocks

OpenWonton 0.12 and earlier allowed a few variations for defining blocks. For example, the following variations of `meta` were accepted:
...
Starting with OpenWonton 1.0 and the HCL2 parser, only the block syntax with unquoted attributes is accepted:
...
Or better yet, the new [`mount`](/nomad/docs/drivers/docker#mount) syntax, introduced in OpenWonton 1.0.1, is more appropriate here:
...
OpenWonton supports multi-line string literals in the so-called "heredoc" style, inspired by Unix shell languages:
...
OpenWonton 0.12 and earlier accepted small decimal values without a leading zero
(e.g. `.3`, `.59`, `.9`). In such case, OpenWonton 1.0 requires a leading zero (e.g.
`0.3`, `0.59`, `0.9`).
