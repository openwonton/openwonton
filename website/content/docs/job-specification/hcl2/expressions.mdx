...
The rest of this page describes all of the features of OpenWonton's
expression syntax.
...
- `null`: a value that represents _absence_ or _omission._ If you set an
  argument to `null`, OpenWonton behaves as though you
...
Whenever the distinction isn't relevant, the OpenWonton documentation uses each
...
Where possible, OpenWonton automatically converts values from one type to
another in order to produce the expected type. If this isn't possible, OpenWonton
will produce a type mismatch error and you must update the configuration with a
more suitable expression.

OpenWonton automatically converts number and bool values to strings when needed.
...
constant value. OpenWonton has a literal expression syntax for each of the value
...
- Strings are usually represented by a double-quoted sequence of Unicode
  ...
  OpenWonton, and have additional documentation on this page:
...
to OpenWonton, such as the `config` attributes in [`task`][task_config],
...
- The `labels` argument (optional) is a list of strings that specifies the block
  ...
  iterator variable in this value. OpenWonton currently only has blocks that support
...
sequence. OpenWonton then processes the following lines until it finds one that
...
To improve on this, OpenWonton also accepts an _indented_ heredoc string variant
...
In this case, OpenWonton analyses the lines in the sequence to find the one
...
In both quoted and heredoc string expressions, OpenWonton supports template
