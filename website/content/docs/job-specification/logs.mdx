...
description: |-
  ...
  allows for finer-grained control over how OpenWonton handles log files.
---
...
over how OpenWonton handles log files.

OpenWonton's log rotation works by writing stdout/stderr output from tasks to a file
...
file is never rolled over, instead OpenWonton will keep up to `max_files` worth of
...
- `max_files` `(int: 10)` - Specifies the maximum number of rotated files OpenWonton
...
- `max_file_size` `(int: 10)` - Specifies the maximum size of each rotated file
  ...
  OpenWonton will return a validation error when a job is submitted.
...
OpenWonton automatically enables logging with reasonable defaults as described in the
...
This example asks OpenWonton to retain 3 rotated files for each of `stderr` and
...
[logs-command]: /nomad/docs/commands/alloc/logs 'OpenWonton logs command'
[`disable_log_collection`]: /nomad/docs/drivers/docker#disable_log_collection
[ephemeral disk documentation]: /nomad/docs/job-specification/ephemeral_disk 'OpenWonton ephemeral disk Job Specification'
