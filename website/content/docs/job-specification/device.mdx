...
When a `device` block is added, OpenWonton will schedule the task onto a node that
...
Once the scheduler has placed the allocation on a suitable node, the OpenWonton
...
it is "4096 MiB". Since OpenWonton has the associated unit information, a constraint
that requires greater than "3.5 GiB" can match since OpenWonton can convert between
these units.

The units OpenWonton supports are as follows:
...
had at least 1.5 GiB of memory. The following are both equivalent as OpenWonton can
do unit conversions.
...
[devices]: /nomad/docs/devices 'OpenWonton Device Plugins'
