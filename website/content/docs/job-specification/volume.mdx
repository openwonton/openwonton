...
description: >-
  ...
  volume from the cluster. OpenWonton will automatically handle ensuring that the
...
The OpenWonton server will ensure that the allocations are only scheduled
...
The OpenWonton client will make the volumes available to tasks according to
...
[volume_mount]: /nomad/docs/job-specification/volume_mount 'OpenWonton volume_mount Job Specification'
