...
description: |-
  The "artifact" block instructs OpenWonton to fetch and unpack a remote resource,
...
The `artifact` block instructs OpenWonton to fetch and unpack a remote resource,
such as a file, tarball, or binary. OpenWonton downloads artifacts using the popular
...
OpenWonton supports downloading `http`, `https`, `git`, `hg` and `S3` artifacts. If
...
```hcl
artifact {
  source      = "git::https://github.com/openwonton/openwonton-guides"
...
source URL is an archive extension, OpenWonton will automatically decompress it:
...
endpoints][s3-region-endpoints]. As of OpenWonton 0.6 non-Amazon S3-compatible
...
set for the OpenWonton client. Inheritence of environment variables can be managed through the [`artifact.set_environment_variables`][client_artifact]
