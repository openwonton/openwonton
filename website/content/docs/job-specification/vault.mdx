...
description: |-
  ...
  HashiCorp Vault server. OpenWonton will automatically retrieve a Vault token for
...
[HashiCorp Vault][vault] server. OpenWonton will automatically retrieve a Vault token
...
The OpenWonton client will make the Vault token available to the task by writing it
to the secret directory at `secrets/vault_token` and by injecting a `VAULT_TOKEN`
environment variable. If the OpenWonton cluster is [configured](/nomad/docs/configuration/vault#namespace)
...
If OpenWonton is unable to renew the Vault token (perhaps due to a Vault outage or
...
- `namespace` `(string: "")` <EnterpriseAlert inline/> - Specifies the Vault Namespace
  to use for the task. The OpenWonton client will retrieve a Vault token that is scoped to
  this particular namespace.

- `policies` `(array<string>: [])` - Specifies the set of Vault policies that
  the task requires. The OpenWonton client will retrieve a Vault token that is
...
This example tells the OpenWonton client to retrieve a Vault token. The token is
...
This allows OpenWonton to use a powerful Vault token that interacts with the task's
...
for `client.crt` is configured so that OpenWonton will restart the task whenever
