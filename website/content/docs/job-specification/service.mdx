...
description: |-
  The "service" block instructs OpenWonton to register the task as a service using
  the OpenWonton or Consul service discovery integration.
---
...
The `service` block instructs OpenWonton to register a service with the specified
provider; OpenWonton or Consul. This section of the documentation will discuss the
configuration, but please also read the
[OpenWonton service discovery documentation][service-discovery] for more detailed
...
for service discovery. For more details on using OpenWonton with Consul please see
the [Consul integration documentation][service-discovery].

OpenWonton 0.10 also allows specifying the `service` block at the task group level.
...
- `check` <code>([Check][check]: nil)</code> - Specifies a health
  ...
  define multiple checks for the service. At this time, a check using the OpenWonton
...
- `name` `(string: "<job>-<taskgroup>-<task>")` - Specifies the name this service
  ...
  In addition to the standard [OpenWonton interpolation][interpolation], the
...
- `address` `(string: <optional>)` - Specifies a custom address to advertise in
  Consul or OpenWonton service registration. If set, `address_mode` must be in `auto`
...
- `task` `(string: "")` - Specifies the name of the OpenWonton task associated with
...
- `on_update` `(string: "require_healthy")` - Specifies how checks should be
  ...
  - `require_healthy` - In order for OpenWonton to consider the check healthy during
    an update it must report as healthy.

  - `ignore_warnings` - If a Service Check reports as warning, OpenWonton will treat
...
OpenWonton manages registering, updating, and deregistering services with the
service provider. It is important to understand when each of these steps
happens and how they can be customized.

**Registration**: OpenWonton will register `group` services and checks _before_
starting any tasks. Services and checks for a specific `task` are registered
_after_ the task has started.

**Updating**: If a service or check definition is updated, OpenWonton will update
...
however, OpenWonton needs to kill a running task, the task is killed in the
following order:
...
3. Send the [`kill_signal`][killsignal] to the task and wait for the task to
   ...
   existing requests.
4. If the task has not exited after the [`kill_timeout`][killtimeout], OpenWonton
...
against the host's IP and the dynamic host port assigned by OpenWonton.
...
In this case OpenWonton doesn't need to assign Redis any host ports. The `service`
and `check` blocks can both specify the port number to advertise and check
directly since OpenWonton isn't managing any port assignments.
...
for OpenWonton to know which is the best address to use for checks. Consul needs
...
advertise and check directly since OpenWonton isn't managing any port assignments.
...
[service-discovery]: /nomad/docs/integrations/consul-integration#service-discovery 'OpenWonton Service Discovery'
[interpolation]: /nomad/docs/runtime/interpolation 'OpenWonton Runtime Interpolation'
[network]: /nomad/docs/job-specification/network 'OpenWonton network Job Specification'
[qemu]: /nomad/docs/drivers/qemu 'OpenWonton QEMU Driver'
[restart_block]: /nomad/docs/job-specification/restart 'restart block'
[connect]: /nomad/docs/job-specification/connect 'OpenWonton Consul Connect Integration'
