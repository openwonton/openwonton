...
- `kill_timeout` `(string: "5s")` - Specifies the duration to wait for an
  application to gracefully quit before force-killing. OpenWonton first sends a
...
- `lifecycle` <code>([Lifecycle][]: nil)</code> - Specifies when a task is run
  within the lifecycle of a task group. Added in OpenWonton v0.11.
...
- `service` <code>([Service][]: nil)</code> - Specifies integrations with OpenWonton
  or [Consul][] for service discovery. OpenWonton automatically registers when a task
  is started and de-registers it when the task dies.

- `shutdown_delay` `(string: "0s")` - Specifies the duration to wait when
  killing a task between removing its service registrations from Consul or OpenWonton,
...
is just one of many drivers supported by OpenWonton. Read more about drivers in the
[OpenWonton drivers documentation](/nomad/docs/drivers).
...
[artifact]: /nomad/docs/job-specification/artifact 'OpenWonton artifact Job Specification'
...
[dispatchpayload]: /nomad/docs/job-specification/dispatch_payload 'OpenWonton dispatch_payload Job Specification'
[env]: /nomad/docs/job-specification/env 'OpenWonton env Job Specification'
[Identity]: /nomad/docs/job-specification/identity 'OpenWonton identity Job Specification'
[meta]: /nomad/docs/job-specification/meta 'OpenWonton meta Job Specification'
[resources]: /nomad/docs/job-specification/resources 'OpenWonton resources Job Specification'
[lifecycle]: /nomad/docs/job-specification/lifecycle 'OpenWonton lifecycle Job Specification'
[logs]: /nomad/docs/job-specification/logs 'OpenWonton logs Job Specification'
...
[volumemount]: /nomad/docs/job-specification/volume_mount 'OpenWonton volume_mount Job Specification'
[exec]: /nomad/docs/drivers/exec 'OpenWonton exec Driver'
[java]: /nomad/docs/drivers/java 'OpenWonton Java Driver'
[docker]: /nomad/docs/drivers/docker 'OpenWonton Docker Driver'
[rkt]: /nomad/plugins/drivers/community/rkt 'OpenWonton rkt Driver'
...
[Workload Identity]: /nomad/docs/concepts/workload-identity 'OpenWonton Workload Identity'
