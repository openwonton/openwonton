...
- `attribute` `(string: "")` - Specifies the name or reference of the attribute
  to examine for the constraint. This can be any of the [OpenWonton interpolated
...
OpenWonton job specification for constraints. The operator is always specified as a
...
- `"semver"` - Specifies a version constraint against the attribute. Only
  ...
  non-prerelease constraints (eg `>= 1.0`). _Added in OpenWonton v0.10.2._
...
[client-meta]: /nomad/docs/configuration/client#custom-metadata-network-speed-and-node-class 'OpenWonton Custom Metadata, Network Speed, and Node Class'
