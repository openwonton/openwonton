...
sidecar or Connect gateway managed by OpenWonton for the [Consul
...
OpenWonton automatically launches and manages an Envoy task for use as a proxy sidecar
...
OpenWonton's version interpolation, e.g.
...
- `user` `(string: nil)` - Determines which user is used to run the task, defaults
  to the same user the OpenWonton client is being run as.
...
- `meta` `(map: nil)` - Arbitrary metadata associated with this task that's opaque to OpenWonton.
...
[connect]: /nomad/docs/job-specification/connect 'OpenWonton connect Job Specification'
...
[sidecar_service]: /nomad/docs/job-specification/sidecar_service 'OpenWonton sidecar service Specification'
