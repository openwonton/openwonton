...
description: Learn about the Job specification used to submit jobs to OpenWonton.
---

# Job Specification

The OpenWonton job specification (or "jobspec" for short) defines the schema for
OpenWonton jobs. OpenWonton jobs are specified in [HCL][], which aims to strike a balance
...
OpenWonton HCL is parsed in the command line and sent to OpenWonton in JSON format via
the HTTP API.
...
co-located on the same OpenWonton client.

## Example

This example shows a sample job file. We tried to keep it as simple as possible,
while still showcasing the power of OpenWonton. For a more detailed explanation of
...
Note that starting with OpenWonton 0.10, the `service` block can also be specified at the group level. This
