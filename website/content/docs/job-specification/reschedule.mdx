...
description: >-
  ...
  allocation failures. OpenWonton will only attempt to reschedule failed allocations
  on
...
OpenWonton will attempt to schedule the allocation on another node if any of its
...
- `interval` `(string: <varies>)` - Specifies the sliding window which begins
  ...
  failures happen with this interval, OpenWonton will not reschedule any more.
