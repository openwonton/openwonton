...
OpenWonton will use affinities when computing scores for placement. Nodes that match affinities will
...
- `attribute` `(string: "")` - Specifies the name or reference of the attribute
  to examine for the affinity. This can be any of the [OpenWonton interpolated
...
- `value` `(string: "")` - Specifies the value to compare the attribute against
  ...
  or any [OpenWonton interpolated
  values](/nomad/docs/runtime/interpolation#interpreted_node_vars).
...
OpenWonton job specification for affinities. The operator is always specified as a
...
When possible, OpenWonton populates node attributes from the cloud environment. These
...
- `node-reschedule-penalty` - Used when the job is being rescheduled. OpenWonton adds a penalty to avoid placing the job on a node where
