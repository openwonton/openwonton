...
description: |-
  The "check_restart" block instructs OpenWonton when to restart tasks with
...
The `check_restart` block instructs OpenWonton when to restart tasks with unhealthy
service checks. When a health check in OpenWonton or Consul has been unhealthy for the `limit`
...
Once the task restarts OpenWonton waits the `grace` period again before starting to
...
[gh-9176]: https://github.com/openwonton/openwonton/issues/9176
