...
The "csi_plugin" block allows the task to specify it provides a
Container Storage Interface plugin to the cluster. OpenWonton will
...
- `id` `(string: <required>)` - This is the ID for the plugin. Some
  ...
  below); you need to use the same ID for both so that OpenWonton knows they
  belong to the same plugin.

- `type` `(string: <required>)` - One of `node`, `controller`, or
  `monolith`. Each plugin supports one or more types. Each OpenWonton
...
- `mount_dir` `(string: <optional>)` - The directory path inside the
  ...
  bidirectional communication with OpenWonton. This field is typically not
...
CSI plugins run as OpenWonton tasks, but after mounting the volume are not in the
...
the CSI plugin. But when an allocation that mounts a volume stops, OpenWonton will
