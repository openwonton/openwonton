...
- `migrate` `(bool: false)` - This specifies that the OpenWonton client should make a
...
- `size` `(int: 300)` - Specifies the size of the ephemeral disk in MB. The
  current OpenWonton ephemeral storage implementation does not enforce this limit;
  however, it is used during job placement.

- `sticky` `(bool: false)` - Specifies that OpenWonton should make a best-effort
...
This example shows enabling sticky volumes with OpenWonton using ephemeral disks:
...
[logs documentation]: /nomad/docs/job-specification/logs 'OpenWonton logs Job Specification'
