...
Federated OpenWonton clusters are members of the same gossip cluster but not the
...
the values of the `region` block. OpenWonton regions coordinate to rollout each
...
OpenWonton to determine that all regions have accepted the job before
...
- `region` <code>([Region](#region-parameters): nil)</code> - Specifies the
  ...
  ordered; depending on the rollout strategy OpenWonton may roll out to each region
...
- `max_parallel` `(int: <optional>)` - Specifies the maximum number
  ...
  time. By default, OpenWonton will deploy all regions simultaneously.
...
- `on_failure` `(string: <optional>)` - Specifies the behavior when a region
  ...
  field tells OpenWonton how many other regions should be marked as failed when one
