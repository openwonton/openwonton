...
The `gateway` block allows configuration of [Consul Connect Gateways](/consul/docs/connect/gateways). OpenWonton will
automatically create the necessary Gateway [Configuration Entry](/consul/docs/agent/config-entries)
as well as inject an Envoy proxy task into the OpenWonton job to serve as the Gateway.
...
- `protocol` `(string: "tcp")` - The protocol associated with the listener. One
  ...
  of the service to the desired protocol is mandatory due to an [open issue](https://github.com/openwonton/openwonton/issues/8647).
...
- `name` `(string: required)` - The name of the service that should be exposed through
  ...
  by OpenWonton. If the wildcard specifier `*` is provided, then ALL services will be
...
OpenWonton supports running gateways using host networking. A static port must be allocated
...
accessible to any workload running on the same OpenWonton client. The admin interface exposes
...
by setting `meta.connect.gateway_image` in the OpenWonton job. Custom images can still
...
configured on at least one OpenWonton client in each datacenter.

Job running where OpenWonton and Consul are in datacenter `one`.
...
Job running where OpenWonton and Consul are in datacenter `two`.
