...
description: |-
  The "check" block declares service check definition for a service registered into the OpenWonton or Consul service provider.
---
...
The `check` block instructs OpenWonton to register a check associated with a [service][service]
into the OpenWonton or Consul service provider.
...
- `initial_status` `(string: <enum>)` - Specifies the starting status of the
  ...
  In the OpenWonton service provider, the initial status of a check is `pending`
  until OpenWonton produces an initial check status result.
...
- `interval` `(string: <required>)` - Specifies the frequency of the health checks
  that Consul or OpenWonton service provider will perform. This is specified using a label
...
- `name` `(string: "service: <name> check")` - Specifies the name of the health
  check. If the name is not specified OpenWonton generates one based on the service name.

- `path` `(string: <varies>)` - Specifies the path of the HTTP endpoint which
  will be queried to observe the health of a service. OpenWonton will automatically
...
- `port` `(string: <varies>)` - Specifies the label of the port on which the
  ...
  ports by default. In OpenWonton 0.7.1 or later numeric ports may be used if
...
- `type` `(string: <required>)` - This indicates the check types supported by
  OpenWonton. For Consul service checks, valid options are `grpc`, `http`, `script`,
  and `tcp`. For OpenWonton service checks, valid options are `http` and `tcp`.
...
service on the IP and port registered with OpenWonton at `/_healthz` every 5 seconds,
...
For checks registered into the OpenWonton service provider, the status information will
indicate `Mode = readiness` for readiness checks and `Mode = healthiness` for health
checks.

### Check status on CLI

For checks registered into the OpenWonton service provider, the status information of
checks can be viewed per-allocation. The `alloc status` command now includes
summary information for OpenWonton service checks.
...
<small>
  {' '}
  Script checks are not supported for the QEMU driver since the OpenWonton client
