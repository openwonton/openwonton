...
A job is a declarative specification of tasks that OpenWonton should run. Jobs have
...
- `constraint` <code>([Constraint][constraint]: nil)</code> -
  ...
  [OpenWonton constraint reference][constraint] for more
  details.
...
- `affinity` <code>([Affinity][affinity]: nil)</code> -
  ...
  [OpenWonton affinity reference][affinity] for more
  details.
...
- `spread` <code>([Spread][spread]: nil)</code> - This can be provided multiple times
  ...
  See the [OpenWonton spread reference][spread] for more details.
...
- `namespace` `(string: "default")` - The namespace in which to execute the job.
  Prior to OpenWonton 1.0 namespaces were Enterprise-only.
...
- `type` `(string: "service")` - Specifies the [OpenWonton scheduler][scheduler] to
  use. OpenWonton provides the `service`, `system`, `batch`, and `sysbatch` (new in
  OpenWonton 1.2) schedulers.
...
This example job executes the `uptime` command on 10 OpenWonton clients in the fleet,
...
secrets to a file on disk, which the application then consumes. OpenWonton handles
...
[parameterized]: /nomad/docs/job-specification/parameterized 'OpenWonton parameterized Job Specification'
[periodic]: /nomad/docs/job-specification/periodic 'OpenWonton periodic Job Specification'
...
[scheduler]: /nomad/docs/schedulers 'OpenWonton Scheduler Types'
