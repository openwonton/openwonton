...
description: |-
  ...
  same OpenWonton client. Any task within a group will be placed on the same client.
---
...
The `group` block defines a series of tasks that should be co-located on the
same OpenWonton client. Any [task][] within a group will be placed on the same
client.
...
- `spread` <code>([Spread][spread]: nil)</code> - This can be provided
  ...
  [OpenWonton spread reference](/nomad/docs/job-specification/spread) for more details.
...
- `reschedule` <code>([Reschedule][]: nil)</code> - Allows to specify a
  rescheduling strategy. OpenWonton will then attempt to schedule the task on another
...
- `service` <code>([Service][]: nil)</code> - Specifies integrations with OpenWonton
  or [Consul](/nomad/docs/configuration/consul) for service discovery. OpenWonton
...
- `shutdown_delay` `(string: "0s")` - Specifies the duration to wait when
  stopping a group's tasks. The delay occurs between Consul or OpenWonton service
...
- `stop_after_client_disconnect` `(string: "")` - Specifies a duration after
  which a OpenWonton client will stop allocations, if it cannot communicate with the
  ...
  "lost" and OpenWonton will schedule replacement allocations. The replaced
  ...
  exclusive access to an external resource. When specified, the OpenWonton client
  will stop them after this duration.
  The OpenWonton client process must be running for this to occur. This setting
  cannot be used with [`max_client_disconnect`].

- `max_client_disconnect` `(string: "")` - Specifies a duration during which a
  OpenWonton client will attempt to reconnect allocations after it fails to heartbeat
...
in OpenWonton, please see the [OpenWonton service discovery documentation][service_discovery].
...
client if the OpenWonton server cluster fails, since the client will be
...
`max_client_disconnect` specifies a duration during which a OpenWonton client will
...
restart. When `max_client_disconnect` is specified, the OpenWonton server will mark
...
OpenWonton will compare the "unknown" allocations with their replacements and keep
...
~> **Note:** The `max_client_disconnect` feature is only supported on OpenWonton
...
`datacenter` of OpenWonton clients where all clients are not 1.3.0 or above is unsupported.
...
[spread]: /nomad/docs/job-specification/spread 'OpenWonton spread Job Specification'
[affinity]: /nomad/docs/job-specification/affinity 'OpenWonton affinity Job Specification'
[ephemeraldisk]: /nomad/docs/job-specification/ephemeral_disk 'OpenWonton ephemeral_disk Job Specification'
...
[meta]: /nomad/docs/job-specification/meta 'OpenWonton meta Job Specification'
[migrate]: /nomad/docs/job-specification/migrate 'OpenWonton migrate Job Specification'
[network]: /nomad/docs/job-specification/network 'OpenWonton network Job Specification'
[reschedule]: /nomad/docs/job-specification/reschedule 'OpenWonton reschedule Job Specification'
[restart]: /nomad/docs/job-specification/restart 'OpenWonton restart Job Specification'
[service]: /nomad/docs/job-specification/service 'OpenWonton service Job Specification'
[service_discovery]: /nomad/docs/integrations/consul-integration#service-discovery 'OpenWonton Service Discovery'
[update]: /nomad/docs/job-specification/update 'OpenWonton update Job Specification'
[vault]: /nomad/docs/job-specification/vault 'OpenWonton vault Job Specification'
[volume]: /nomad/docs/job-specification/volume 'OpenWonton volume Job Specification'
