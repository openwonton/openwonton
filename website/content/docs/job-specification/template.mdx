...
description: |-
  ...
  configurations within a OpenWonton task.
---
...
OpenWonton utilizes [Go template][gt] and a tool called [Consul Template][ct], which
adds a set of new functions that can be used to retrieve data from Consul and
Vault. OpenWonton templates can reference [OpenWonton's runtime
environment variables][env], [node attributes and metadata][nodevars],
[OpenWonton service registrations][ct_api_nsvc], and [OpenWonton variables][nvars].
...
- `change_mode` `(string: "restart")` - Specifies the behavior OpenWonton should take
  if the rendered template changes. OpenWonton will always write the new contents of
  the template to the specified destination. The following possible values describe
  OpenWonton's action after writing the template to disk.
...
- `uid` `(int: nil)` - Specifies the rendered template owner's user ID. If
  negative or not specified (`nil`) the ID of the OpenWonton agent user will be used.
...
- `gid` `(int: nil)` - Specifies the rendered template owner's group ID. If
  negative or not specified (`nil`) the ID of the OpenWonton agent group will be
  used.
...
semantics of OpenWonton templates.
...
function to ensure special characters are properly parsed by OpenWonton.
...
`toJSON` function to ensure OpenWonton passes the password to the task unchanged.
...
## OpenWonton Integration

### OpenWonton Services

~> OpenWonton Services are new in OpenWonton 1.3.

OpenWonton service registrations can be queried using the `nomadService` and
...
### Simple Load Balancing with OpenWonton Services

~> Simple load balancing with OpenWonton Services is new in OpenWonton 1.3.2.
...
configuration files, triggering fewer restarts and signaling of OpenWonton tasks.
...
### OpenWonton Variables

OpenWonton [variables] can be queried using the `nomadVar`, `nomadVarList`,
`nomadVarListSafe`, and `nomadVarExists` functions.

#### `nomadVarList` and `nomadVarListSafe`

These functions can be used to list the paths of OpenWonton variables available to
...
These functions can be used to a read OpenWonton variable, assuming the task has
...
This function can be used to check if a OpenWonton variable exists at the provided
...
These are three different input templates, but when run under the OpenWonton job,
...
OpenWonton jobs will need to grant permissions to `read` under `secret/data/...`
rather than `secret/...`.
...
- `function_denylist` `([]string: ["plugin"])` - Specifies a list of template
  ...
  the host as root (unless OpenWonton is configured to run as a non-root user).
...
[`changescript`]: /nomad/docs/job-specification/change_script 'OpenWonton change_script Job Specification'
...
[nvars]: /nomad/docs/concepts/variables 'OpenWonton Variables'
...
[env]: /nomad/docs/runtime/environment 'OpenWonton Runtime Environment'
[nodevars]: /nomad/docs/runtime/interpolation#interpreted_node_vars 'OpenWonton Node Variables'
