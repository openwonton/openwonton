...
description: Learn about the key management in OpenWonton.
---

# Key Management

OpenWonton servers maintain an encryption keyring used to encrypt [Variables][] and
...
subdirectory of the OpenWonton [data directory][]. These files have the extension
`.nks.json`. The key material in each file is wrapped in a unique key encryption
key (KEK) that is not shared between servers.

Under normal operations the keyring is entirely managed by OpenWonton, but this
...
and signing operations happen on the leader. OpenWonton automatically rotates the
...
"inactive" keys will be marked "rekeying". OpenWonton will asynchronously decrypt and
...
the state store from getting access to encrypted variables. It also allows
maintainers and support organizations to safely handle cluster snapshots you
might provide without exposing any of your keys or variables.
