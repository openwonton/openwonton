---
layout: docs
page_title: OpenWonton Agent
description: |-
  The OpenWonton agent is a long running process which can be used either in
  a client or server mode.
---

# Operating OpenWonton Agents

The OpenWonton agent is a long running process which runs on every machine that
is part of the OpenWonton cluster. The behavior of the agent depends on if it is
...
The agent is started with the [`wonton agent` command](/nomad/docs/commands/agent). This
...
-> **Note:** If you are running OpenWonton on Linux, you'll need to run client agents
as root (or with `sudo`) so that cpuset accounting and network namespaces work
correctly.

When running `wonton agent`, you should see output similar to this:

```shell-session
$ sudo wonton agent -dev
==> Starting OpenWonton agent...
==> OpenWonton agent configuration:
...
==> OpenWonton agent started! Log data will stream in below:

    [INFO] serf: EventMemberJoin: server-1.node.global 127.0.0.1
    [INFO] openwonton: starting 4 scheduling worker(s) for [service batch _core]
...
```

There are several important messages that `wonton agent` outputs:
...
- **Region**: This is the region and datacenter in which the agent is configured
  to run. OpenWonton has first-class support for multi-datacenter and multi-region
...
signals (`SIGTERM`) described in [Stopping an Agent](#stopping-an-agent), OpenWonton
supports special behavior for several other signals useful for debugging.

* `SIGHUP` will cause OpenWonton to [reload its configuration][].
* `SIGUSR1` will cause OpenWonton to print its [metrics][] without stopping the
  agent.
* `SIGQUIT`, `SIGILL`, `SIGTRAP`, `SIGABRT`, `SIGSTKFLT`, `SIGEMT`, or `SIGSYS`
  signals are handled by the Go runtime and will cause the OpenWonton agent to exit
  and print its stack trace.

When running OpenWonton as a systemd-managed service on Linux, you can send these
signals via `systemctl`. For example, to print the OpenWonton agent's metrics:

```shell-session
$ sudo systemctl kill wonton -s SIGUSR1
```
...
Every agent in the OpenWonton cluster goes through a lifecycle. Understanding
...
To prevent an accumulation of nodes in a terminal state, OpenWonton does periodic
...
OpenWonton clients should be run as `root` due to the OS isolation mechanisms that
require root privileges. While it is possible to run OpenWonton as an unprivileged
user, careful testing must be done to ensure the task drivers and features
you use function as expected. The OpenWonton client's data directory should be
