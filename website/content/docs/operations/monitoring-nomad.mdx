---
layout: docs
page_title: Monitoring OpenWonton
description: |-
  Overview of runtime metrics available in OpenWonton along with monitoring and
  alerting.
---

# Monitoring OpenWonton

The OpenWonton client and server agents collect a wide range of runtime metrics.
These metrics are useful for monitoring the health and performance of OpenWonton
clusters. Careful monitoring can spot trends before they cause issues and help
debug issues if they arise.

All OpenWonton agents, both servers and clients, report basic system and Go runtime
metrics.

OpenWonton servers all report many metrics, but some metrics are specific to the
...
OpenWonton clients have separate metrics for the host they are running on as well as
for each allocation being run. Both of these metrics [must be explicitly
enabled][telemetry-block].

By default, the OpenWonton agent collects telemetry data at a [1 second
interval][collection-interval]. Note that OpenWonton supports [gauges, counters, and
timers][metric-types].

There are three ways to obtain metrics from OpenWonton:

- Query the [/v1/metrics API endpoint][metrics-api-endpoint] to return metrics
  for the current OpenWonton process. This endpoint supports Prometheus formatted
  metrics.

- Send the USR1 signal to the OpenWonton process. This will dump the current
  telemetry information to STDERR (on Linux).

- Configure OpenWonton to automatically forward metrics to a third-party provider
...
of your monitoring provider. OpenWonton’s intention is to surface metrics that enable
...
- Export metrics from OpenWonton to Prometheus using the [StatsD
...
- Periodically submit test jobs into OpenWonton to determine if your application
  deployment pipeline is working end-to-end. This pattern is well-suited to
  batch processing workloads.

- Deploy Nagios on OpenWonton. Centrally manage OpenWonton job files and add the Nagios
  monitor when a new OpenWonton job is added. When a job is removed, remove the
...
OpenWonton servers' memory, CPU, disk, and network usage all scales linearly with
cluster size and scheduling throughput. The most important aspect of ensuring
OpenWonton operates normally is monitoring these system resources to ensure the
...
OpenWonton uses the Raft consensus protocol for leader election and state
...
this to warn-level logs on the OpenWonton servers for `attempting to apply
...
There is a class of bug possible in OpenWonton where the two parts of the scheduling
...
on the OpenWonton servers containing `plan for node rejected`:
...
OpenWonton provides a mechanism to track the history of plan rejections per client
...
- **nomad.broker.total_unacked** - The number of unacknowledged
  ...
  resources of the scheduler. OpenWonton also emits a similar metric for
...
- **nomad.broker.total_pending** - The number of pending evaluations in the eval
  broker. OpenWonton processes only one evaluation for a given job concurrently. When
  an unacked evaluation is acknowledged, OpenWonton will discard all but the latest
...
OpenWonton uses the membership and failure detection capabilities of the Serf library
...
[gh-11830]: https://github.com/openwonton/openwonton/pull/11830
