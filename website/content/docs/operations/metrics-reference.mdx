...
description: Learn about the different metrics available in OpenWonton.
---

# Metrics Reference

The OpenWonton agent collects various runtime metrics about the performance of
...
OpenWonton process. This data is available via HTTP at `/metrics`. See
[Metrics](/nomad/api-docs/metrics) for more information.

To view this data via sending a signal to the OpenWonton process: on Unix,
this is `USR1` while on Windows it is `BREAK`. Once OpenWonton receives the signal,
...
getting a better view of what OpenWonton is doing.
...
OpenWonton emits metrics in a tagged format. Each metric can support more than one
tag, meaning that it is possible to do a match over metrics for datapoints
such as a particular datacenter, and return all metrics with this tag. OpenWonton
supports labels for namespaces as well.

## Key Metrics

The metrics in the table below are the most important metrics for monitoring
the overall health of a OpenWonton cluster.
...
| `nomad.nomad.heartbeat.active`               | Number of active heartbeat timers. Each timer represents a OpenWonton Client connection                                                                                                                                | # of heartbeat timers          | Gauge   |
| `nomad.nomad.heartbeat.invalidate`           | The length of time it takes to invalidate a OpenWonton Client due to failed heartbeats                                                                                                                                 | ms / Heartbeat Invalidation    | Timer   |
...
The OpenWonton client emits metrics related to the resource usage of the allocations
...
[periodic](/nomad/docs/job-specification/parameterized) jobs. OpenWonton
...
OpenWonton will emit [tagged metrics][tagged-metrics], in the below format:
...
| `nomad.client.uptime`                     | Uptime of the host running the OpenWonton client                                          | Seconds    | Gauge   | datacenter, host, node_class, node_id, node_pool, node_scheduling_eligibility, node_status       |
...
Job summary metrics are emitted by the OpenWonton leader server.
...
Job status metrics are emitted by the OpenWonton leader server.
...
Agent metrics are emitted by all OpenWonton agents running in either client or server mode.
