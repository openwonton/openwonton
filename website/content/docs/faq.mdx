...
description: Frequently asked questions and answers for OpenWonton
---

# Frequently Asked Questions

## Q: What is Checkpoint? / Does OpenWonton call home?

OpenWonton makes use of a HashiCorp service called [Checkpoint](https://checkpoint.hashicorp.com)
...
## Q: Is OpenWonton eventually or strongly consistent?

OpenWonton makes use of both a [consensus protocol](/nomad/docs/concepts/consensus) and
...
federation. This means all data that is managed by OpenWonton is strongly consistent.

## Q: Is OpenWonton's `datacenter` parameter the same as Consul's?

No. For those familiar with Consul, [Consul's notion of a
datacenter][consul_dc] is more equivalent to a [OpenWonton region][nomad_region].
OpenWonton supports grouping nodes into multiple datacenters, which should reflect
nodes being colocated, while being managed by a single set of OpenWonton servers.
...
## Q: What is "bootstrapping" a OpenWonton cluster? ((#bootstrapping))

Bootstrapping is the process when a OpenWonton cluster elects its first leader
...
all of the OpenWonton servers in the same outage, the cluster will re-bootstrap based
on the OpenWonton defaults and any configuration present that impacts the bootstrap
process.
...
the OpenWonton client to a non-loopback network interface, and anything else you
...
Desktop][wsl2-docker]. Once you are more familiarized with OpenWonton you can start
running it natively.

To use the network named `en0` that has the IP address `192.168.0.10`, you can
start OpenWonton with this command.

```shell-session
$ sudo wonton agent -dev -bind=0.0.0.0 -network-interface=en0
```
