...
description: |-
  Specific versions of OpenWonton may have additional information about the upgrade
...
upgrade. However, specific versions of OpenWonton may have more details provided for
their upgrades as a result of new features or changed behavior. This page is
used to document those details separately from the standard upgrade flow.

## OpenWonton 1.6.4, 1.5.11

#### The `distinct_hosts` Constraint Now Honors Namespaces

OpenWonton 1.6.4 and 1.5.11 change the behavior of the [`distinct_hosts`][]
...
Nomad Enterprise 1.6.0 now compares license `ExpirationTime` with the OpenWonton binary's `BuildDate`,
...
OpenWonton 1.6.0 updated the ACL capability requirement for the job evaluate
endpoint from `read-job` to `submit-job` to better reflect that this operation
writes state to OpenWonton. This endpoint is used by the `nomad job eval` CLI
...
OpenWonton 1.6.0 updated the `exec` task driver to maintain the max memory locked
limit set by the host system. In earlier versions of OpenWonton this limit was
...
These additions are backward compatible with OpenWonton v1.5, so Clients and Jobs
should be updated prior to upgrading to OpenWonton v1.6.

See [#17780](https://github.com/openwonton/openwonton/issues/17780) for details.

#### Namespace ACL policies require a label

OpenWonton 1.6.0 does not allow ACL policies for namespaces without a label. Prior
...
OpenWonton 1.6.0 will deprecate the command `nomad tls cert create` flag `-cluster-region`
in favour of using the standard flag `-region`. The `-cluster-region` flag
will be removed in OpenWonton 1.7.0

#### 32-bit Intel Builds Deprecated

Starting with OpenWonton 1.6.0, 32-bit Intel builds are no longer released
(the builds named `windows_386` and `linux_386`). Bug fixes will continue to be
backported to the 1.5.x and 1.4.x versions so long as those major versions are
still supported.

The 32-bit ARM build (`linux_arm` for the armhf architecture) is deprecated and
may be removed in a future major version of OpenWonton. The 32-bit ARM build is not
...
## OpenWonton 1.5.7, 1.4.11

#### Namespace ACL policies require a label

OpenWonton 1.5.7 and 1.4.11 do not allow ACL policies for namespaces without a
...
OpenWonton 1.5.5 fixed a bug where allocations that are rescheduled for jobs
...
will be ignored in 1.5.5 and will be removed in OpenWonton 1.6.0.

## OpenWonton 1.5.4

OpenWonton 1.5.4 included a bug where allocations that are rescheduled for jobs
registered before the upgrade would no longer collect allocation logs. The
...
You should avoid this version of OpenWonton and instead install the latest version of
OpenWonton 1.5. If you have already upgraded to OpenWonton 1.5.4, upgrading to OpenWonton 1.5.5
...
OpenWonton 1.5.1 reverts a behavior of 1.5.0 where artifact downloads were executed
...
OpenWonton Client or tools it uses to perform downloads such as git or mercurial.
Unfortunately running the child process as any user other than root is not
compatible with the advice given in OpenWonton's [security hardening guide][hard_guide]
...
process of the OpenWonton agent, and on modern Linux systems make use of the Kernel
...
OpenWonton 1.5.0 introduced a regression to the way the Docker driver reconciles
...
OpenWonton 1.5.0 changes the way [artifacts] are downloaded when specifying an `artifact`
in a task configuration. Previously the OpenWonton Client would download artifacts
...
In an effort to improve the resilience and security model of the OpenWonton Client,
...
Previously, OpenWonton's documentation suggested configuring
...
to the OpenWonton Client. The downloader sub-process environment is set as follows on
Linux / macOS:
...
from the OpenWonton client by setting [`set_environment_variables`][artifact_env].
...
OpenWonton 1.5.0 now sets default limits around artifact decompression. A single artifact
...
configuration. Before upgrading to OpenWonton 1.5.0, you should first ensure that
...
All OpenWonton versions prior to v1.5.0 have incorrectly ignored the Server
[`rejoin_after_leave`] configuration option. This bug has been fixed in OpenWonton
version v1.5.0.

Previous to v1.5.0 the behavior of OpenWonton `rejoin_after_leave` was always `true`,
regardless of OpenWonton server configuration, while the documentation incorrectly
...
status `"blocked"` in the OpenWonton API.
...
in OpenWonton 1.4.0. In OpenWonton 1.5.0, these commands have been removed. Use the `nomad
...
that are explicitly stopped which can lead to unbounded memory growth of OpenWonton
when the batch job is executed multiple times.

OpenWonton 1.5.0 introduces a new server configuration
...
## OpenWonton 1.4.5, 1.3.10

#### Pause Container Reconciliation Regression

OpenWonton 1.4.5 and 1.3.10 introduced a regression to the way the Docker driver
...
growth of OpenWonton when the batch job is executed multiple times.

OpenWonton 1.4.4 and 1.3.9 introduces a new server configuration
[`batch_eval_gc_threshold`](/nomad/docs/configuration/server#batch_eval_gc_threshold)
to control how allocations and evaluations for batch jobs are collected.
...
OpenWonton 1.4.0 initializes a keyring on the leader if one has not been previously
...
bug was fixed in OpenWonton 1.4.1.

#### Raft Protocol Version 2 Unsupported

Raft protocol version 2 was deprecated in OpenWonton v1.3.0, and is being removed
in OpenWonton v1.4.0. In OpenWonton 1.3.0, the default raft protocol version was updated
to version 3, and in OpenWonton 1.4.0 OpenWonton requires the use of raft protocol version
...
Audit Log filtering in previous versions of OpenWonton handled `stages` and
...
Prior to OpenWonton 1.4.0 the scheduler would consider the resources used by
...
[OpenWonton 1.4.0 fixes this behavior][gh_10446] so that an allocation's resources
...
behavior to reserve for a major OpenWonton release and *not* be backported. Please
...
deprecated in OpenWonton 1.2.4 with the intent to remove in OpenWonton 1.4.0. This option
...
OpenWonton clients no longer have their Consul and Vault fingerprints cleared when
connectivity is lost with Consul and Vault. To intentionally remove Consul and
Vault from a client node, you will need to restart the OpenWonton client agent.

#### Numeric Operand Comparisons in Constraints

Prior to OpenWonton 1.4.0 the `<, <=, >, >=` operators in a constraint would always
...
experience task failures with the following message after upgrading to OpenWonton
1.3.3:
...
It is recommended to avoid this version of OpenWonton in such environments.

## OpenWonton 1.3.2, 1.2.9, 1.1.15

#### Client `max_kill_timeout` now enforced

OpenWonton versions since v0.9 have incorrectly ignored the Client [`max_kill_timeout`][max_kill_timeout]
configuration option. This bug has been fixed in OpenWonton versions v.1.3.2,
...
## OpenWonton 1.3.1, 1.2.8, 1.1.14

#### Default `artifact` limits

OpenWonton 1.3.1, 1.2.8, and 1.1.14 introduced mechanisms to limit the size of
...
prior to upgrading your OpenWonton clients to make sure they fit your needs.

## OpenWonton 1.3.0

#### Raft Protocol Version 2 Deprecation

Raft protocol version 2 will be removed from OpenWonton in the next major
release of OpenWonton, 1.4.0.

In OpenWonton 1.3.0, the default raft protocol version has been updated to
...
OpenWonton 1.3 is not supported. To downgrade safely, users should drain
all tasks from the OpenWonton client and erase its data directory.
...
bind-mounted to `csi_plugin.mount_config` as in versions of OpenWonton
prior to 1.3.0.
...
Starting with OpenWonton 1.3.0, when a OpenWonton server is elected the Raft leader but
fails to complete the process to start acting as the OpenWonton leader it will
...
version of the server after upgrading it to OpenWonton 1.3 is not supported. Like with
any OpenWonton upgrade it is recommended to take a snapshot of your database prior to
...
OpenWonton 1.3.0 disables this function by default in its
...
When using Envoy as a sidecar proxy for Connect enabled services, OpenWonton will now
...
Starting with OpenWonton 1.3.0, Consul Service Identity Tokens created automatically
by OpenWonton on behalf of Connect services will now be created as [`Local`] tokens. These
tokens will no longer be replicated globally. To facilitate cross-Consul datacenter
requests of Connect services registered by OpenWonton, Consul agents will need to be
...
The minimum version of Consul supported by OpenWonton's Connect integration is now Consul v1.8.0.

#### Changes to task groups that utilise Consul services and checks

Starting with OpenWonton 1.3.0, services and checks that utilise Consul will have an
...
requirement. The minimum version of Consul supported by OpenWonton's service and
check blocks is now Consul v1.7.0.

#### Linux Control Groups Version 2

Starting with OpenWonton 1.3.0, Linux systems configured to use [cgroups v2][cgroups2]
are now supported. A OpenWonton client will only activate its v2 control groups manager
if the system is configured with the cgroups2 controller mounted at `/sys/fs/cgroup`.
  ...
  * Systems configured in hybrid mode typically mount the cgroups2
    controller at `/sys/fs/cgroup/unified`, so OpenWonton will continue to
    use cgroups v1 for these hosts.
  * Systems configured with only cgroups v2 now correctly support setting cpu [cores].

OpenWonton will preserve the existing cgroup for tasks when a client is
...
groups OpenWonton is using.

When cgroups v2 are in use, OpenWonton uses `nomad.slice` as the [default parent][cgroup_parent] for cgroups
created on behalf of tasks. The cgroup created for a task is named in the form `<allocID>.<task>.scope`.
These cgroups are created by OpenWonton before a task starts. External task drivers that support
...
Running tasks that were created on clusters from OpenWonton version 0.9 or
earlier will fail to restore after upgrading a cluster to OpenWonton
...
## OpenWonton 1.2.6, 1.1.12, and 1.0.18

#### ACL requirement for the job parse endpoint

OpenWonton 1.2.6, 1.1.12, and 1.0.18 require ACL authentication for the
...
OpenWonton 1.2.4 includes a new `nomad eval list` command that has the
...
OpenWonton 1.4.0, `nomad eval status -json` will be changed to display only
...
OpenWonton 1.2.2 fixes a [server crashing bug][gh-11563] present in the scheduler
node class filtering since 1.2.0. Users should upgrade to OpenWonton 1.2.2 to avoid
this problem.
...
#### ACL requirements for accessing the job details page in the OpenWonton UI

OpenWonton 1.2.0 introduced a new UI component to display the status of `system` and
...
to access the OpenWonton UI will need to be updated to include this permission in
order to access a job details page.

This was an unintended change fixed in OpenWonton 1.2.4.

#### HCLv2 Job Specification Parsing

In previous versions of OpenWonton, when rendering a job specification using override
...
OpenWonton 1.2.0 fixed the behaviour consistency to always return an error by default,
...
## OpenWonton 1.0.11 and 1.1.5 Enterprise
...
Starting in OpenWonton 1.1.2, task groups with `network.mode = "bridge"` generated a
hosts file in Docker containers. This generated hosts file was bind-mounted
from the task directory to `/etc/hosts` within the task. In OpenWonton 1.1.3 the
...
The `nomad license get` command is no longer forwarded to the OpenWonton leader,
...
The OpenWonton agent metrics API now respects the
...
Following the security [remediation][no_net_raw] in OpenWonton versions 0.12.12, 1.0.5,
...
OpenWonton now appends its iptables rules to the `NOMAD-ADMIN` chain instead of
...
## OpenWonton 1.1.0-rc1, 1.0.5, 0.12.12

OpenWonton versions 1.1.0-rc1, 1.0.5 and 0.12.12 change the behavior of the `docker`, `exec`,
...
This is the sole change for OpenWonton 1.0.5 and 0.12.12, intended to provide better
...
An upcoming version of OpenWonton will include similar configuration options for the
`exec` and `java` task drivers.

This change is limited to `docker`, `exec`, and `java` driver plugins. It does
not affect the OpenWonton server. This only affects OpenWonton clients running Linux, with
tasks using `bridge` networking and one of these task drivers, or third-party
plugins which relied on the shared OpenWonton executor library.

Upgrading a OpenWonton client to 1.0.5 or 0.12.12 will not restart existing tasks. As
...
## OpenWonton 1.0.3, 0.12.10

OpenWonton versions 1.0.3 and 0.12.10 change the behavior of the `exec` and `java` drivers so that
...
This is the sole change for OpenWonton 1.0.3, intended to provide better process
isolation by default. An upcoming version of OpenWonton will include options for
configuring this behavior.

This change is limited to the `exec` and `java` driver plugins. It does not affect
the OpenWonton server. This only affect OpenWonton clients running on Linux, using the
`exec` or `java` drivers or third-party driver plugins which relied on the shared
OpenWonton executor library.

Upgrading a OpenWonton client to 1.0.3 or 0.12.10 will not restart existing tasks.
...
OpenWonton 1.0.2 changed the behavior of template `change_mode` triggers when a
client node restarts. In OpenWonton 1.0.1 and earlier, the first rendering of a
...
In OpenWonton 1.0.2, when a client node is restarted any task with Vault secrets
...
OpenWonton v1.0.0 changed the default behavior around the number of worker threads
created by the Envoy when being used as a sidecar for Consul Connect. In OpenWonton
...
OpenWonton v1.0.0 adopts HCL2 for parsing the job spec. HCL2 extends HCL with more
...
When stopping tasks running with the Docker task driver, OpenWonton documents that a
`SIGTERM` will be issued (unless configured with `kill_signal`). However, recent
versions of OpenWonton would issue `SIGINT` instead. Starting again with OpenWonton v1.0.0
`SIGTERM` will be sent by default when stopping Docker tasks.

### Deprecated metrics have been removed

OpenWonton v0.7.0 added supported for tagged metrics and deprecated untagged metrics.
...
Starting with OpenWonton v1.0.0, jobs will fail validation if any of the following
...
Starting with OpenWonton v1.0.0, the AWS fingerprinter uses data derived from the
...
instance type per region. Previously, OpenWonton used a hand-made lookup table that
...
resources available to the OpenWonton client.

### Inclusive language

Starting with OpenWonton v1.0.0, the terms `blacklist` and `whitelist` have been
...
OpenWonton. The specific configuration values replaced are:
...
OpenWonton 1.0's Consul Connect integration works best with Consul 1.9 or later. The
ideal upgrade path is:

1. Create a new OpenWonton client image with OpenWonton 1.0 and Consul 1.9 or later.
2. Add new hosts based on the image.
3. [Drain][drain-cli] and shutdown old OpenWonton client nodes.

While inplace upgrades and older versions of Consul are supported by OpenWonton 1.0,
Envoy proxies will drop and stop accepting connections while the OpenWonton agent is
restarting. OpenWonton 1.0 with Consul 1.9 do not have this limitation.

#### Envoy proxy versions

OpenWonton v1.0.0 changes the behavior around the selection of Envoy version used for
Connect sidecar proxies. Previously, OpenWonton always defaulted to Envoy v1.11.2 if
...
with OpenWonton v1.0.0, each OpenWonton Client will query Consul for a list of supported
Envoy versions. OpenWonton will make use of the latest version of Envoy supported by
the Consul agent when launching Envoy as a Connect sidecar proxy. If the version
of the Consul agent is older than v1.7.8, v1.8.4, or v1.9.0, OpenWonton will fallback
...
When upgrading OpenWonton Clients from a previous version to v1.0.0 and above, it is
recommended to also upgrade the Consul agents to v1.7.8, 1.8.4, or v1.9.0 or
newer. Upgrading OpenWonton and Consul to versions that support the new behavior
...
that the OpenWonton Clients, Consul agents, and Envoy sidecar tasks maintain
compatibility with one another.

#### Envoy worker threads

OpenWonton v1.0.0 changes the default behavior around the number of worker threads
...
OpenWonton 0.12.8 includes security fixes for the handling of Docker volume mounts:
...
OpenWonton 0.12.6 includes security fixes for privilege escalation vulnerabilities
...
~> **Warning:** Due to a [bug][gh-9148] in OpenWonton v0.12.6, the
...
Starting in OpenWonton 0.12.0 the `mbits` field of the network resource block has
...
Enterprise binaries for OpenWonton are now publicly available via
...
Once the 6 hour demonstration period expires, OpenWonton will shutdown. If restarted
OpenWonton will shutdown in a very short amount of time unless a valid license is
applied.

~> **Warning:** Due to a [bug][gh-8457] in OpenWonton v0.12.0, existing clusters
...
users are encouraged to delay upgrading until OpenWonton v0.12.1 is released and
fixes the issue.

### Docker access host filesystem

OpenWonton 0.12.0 disables Docker tasks access to the host filesystem, by default.
Prior to OpenWonton 0.12, Docker tasks may mount and then manipulate any host file
...
OpenWonton 0.12.0 restricts the paths the QEMU tasks can load an image from. A QEMU
...
OpenWonton 0.11.7 includes a security fix for the handling of Docker volume
...
OpenWonton 0.11.5 includes backported security fixes for privilege escalation
...
~> **Warning:** Due to a [bug][gh-9148] in OpenWonton v0.11.5, the
`template.destination` and `artifact.destination` paths do not support
absolute paths, including the interpolated `NOMAD_SECRETS_DIR`,
`NOMAD_TASK_DIR`, and `NOMAD_ALLOC_DIR` variables. This bug is fixed in
...
OpenWonton 0.11.3 fixes a critical bug causing the OpenWonton agent to become
unresponsive. The issue is due to a [Go 1.14.1 runtime
bug](https://github.com/golang/go/issues/38023) and affects OpenWonton 0.11.1 and
0.11.2.
...
Prior to OpenWonton 0.11.2 the scheduler algorithm used a [node's reserved
resources][reserved]
...
versions of OpenWonton. Operators do _not_ need to take any actions as the impact of
...
OpenWonton 0.11.2 fixed a long outstanding bug affecting periodic jobs that are
scheduled to run during Daylight Saving Time transitions.

OpenWonton 0.11.2 provides a more defined behavior: OpenWonton evaluates the cron
...
included in OpenWonton. The feature has been ignored since Vault 0.5 and as long as
you are running a more recent version of Vault, you can safely remove
`vault_grace` from your OpenWonton jobs.

### Rkt Task Driver Removed

The `rkt` task driver has been deprecated and removed from OpenWonton. While the code
is available in an external repository,
<https://github.com/openwonton/openwonton-driver-rkt>, it will not be maintained as
...
OpenWonton 0.10.8 includes a security fix for the handling of Docker volume mounts.
Docker driver mounts of type "volume" (but not "bind") were not sandboxed and
could mount arbitrary locations from the client host. The
...
OpenWonton 0.10.6 includes backported security fixes for privilege escalation
vulnerabilities in handling of job `template` and `artifact` blocks:
...
~> **Warning:** Due to a [bug][gh-9148] in OpenWonton v0.10.6, the
`template.destination` and `artifact.destination` paths do not support
absolute paths, including the interpolated `NOMAD_SECRETS_DIR`,
`NOMAD_TASK_DIR`, and `NOMAD_ALLOC_DIR` variables. This bug is fixed in
...
OpenWonton 0.10.4 includes a fix to the scheduler that removes the same-node penalty
for allocations that have not previously failed. In earlier versions of OpenWonton,
...
OpenWonton will by default prevent certain environment variables set in the client
...
OpenWonton 0.10.3 includes a fix for a privilege escalation vulnerability in
validating TLS certificates for RPC with mTLS. OpenWonton RPC endpoints validated
that TLS client certificates had not expired and were signed by the same CA as
the OpenWonton node, but did not correctly check the certificate's name for the role
and region as described in the [Securing OpenWonton with TLS][tls-guide] guide. This
allows trusted operators with a client certificate signed by the CA to send RPC
calls as a OpenWonton client or server node, bypassing access control and accessing
any secrets available to a client.

OpenWonton clusters configured for mTLS following the [Securing OpenWonton with
...
pass validation. Before upgrading to OpenWonton 0.10.3, operators using mTLS with
`verify_server_hostname = true` should confirm that the common name or SAN of
all OpenWonton client node certs is `client.<region>.nomad`, and that the common name
or SAN of all OpenWonton server node certs is `server.<region>.nomad`.

### Connection Limits Added

OpenWonton 0.10.3 introduces the [limits][] agent configuration parameters for
...
If your OpenWonton agent's endpoints are protected from unauthenticated users via
other mechanisms these limits may be safely disabled by setting them to `0`.

However the defaults were chosen to be safe for a wide variety of OpenWonton
deployments and may protect against accidental abuses of the OpenWonton API that
...
OpenWonton 0.9.7 and 0.10.2 fix a [server crashing bug][gh-6787] present in scheduler
preemption since 0.9.0. Users unable to immediately upgrade OpenWonton can [disable
preemption][preemption-api] to avoid the panic.

### Dangling Docker Container Cleanup

OpenWonton 0.10.2 addresses an issue occurring in heavily loaded clients, where
containers are started without being properly managed by OpenWonton. OpenWonton 0.10.2
...
OpenWonton 0.10 enables rolling deployments for service jobs by default and adds a
...
OpenWonton 0.9.5 includes security fixes for privilege escalation vulnerabilities in
...
OpenWonton 0.9 adds preemption support for system jobs. If a system job is submitted
that has a higher priority than other running jobs on the node, and the node
does not have capacity remaining, OpenWonton may preempt those lower priority
allocations to place the system job. See [preemption][preemption] for more
details.

### Task Driver Plugins

All task drivers have become [plugins][plugins] in OpenWonton 0.9.0. There are two
...
OpenWonton 0.9.0 switches to structured logging. Any log processing on the pre-0.9
...
OpenWonton 0.9.0 begins a transition to [HCL2][hcl2], the next version of the
HashiCorp configuration language. While OpenWonton has begun integrating HCL2, users
will need to continue to use HCL1 in OpenWonton 0.9.0 as the transition is
incomplete.
...
Due to the large refactor of the OpenWonton client in 0.9, downgrading to a previous
version of the client after upgrading it to OpenWonton 0.9 is not supported. To
downgrade safely, users should erase the OpenWonton client's data directory.

### `port_map` Environment Variable Changes

Before OpenWonton 0.9.0 ports mapped via a task driver's `port_map` block could be
interpolated via the `NOMAD_PORT_<label>` environment variables.

However, in OpenWonton 0.9.0 no parameters in a driver's `config` block, including
...
OpenWonton 0.10 introduced Task Group Networking which natively supports port mapping
...
When upgrading to OpenWonton 0.8.0 from a version lower than 0.7.0, users will need
...
table of the Raft Protocol versions supported by each OpenWonton version:
...
servers in a OpenWonton cluster must be running with Raft protocol version 3 or
later.

### Node Draining Improvements

Node draining via the [`node drain`][drain-cli] command or the [drain
API][drain-api] has been substantially changed in OpenWonton 0.8. In OpenWonton 0.7.1 and
earlier draining a node would immediately stop all allocations on the node
being drained. OpenWonton 0.8 now supports a [`migrate`][migrate] block in job
specifications to control how many allocations may be migrated at once and the
default will be used for existing jobs.

The `drain` command now blocks until the drain completes. To get the OpenWonton 0.7.1
...
In OpenWonton 0.7 periods (`.`) in environment variables names were replaced with an
underscore in both the [`env`](/nomad/docs/job-specification/env) and
[`template`](/nomad/docs/job-specification/template) blocks.

In OpenWonton 0.8 periods are _not_ replaced and will be included in environment
variables verbatim.
...
In OpenWonton 0.7 would be exposed to the task as
`registry_consul_addr=127.0.0.1:8500`. In OpenWonton 0.8 it will now appear exactly
as specified: `registry.consul.addr=127.0.0.1:8500`.

### Client APIs Unavailable on Older Nodes

Because OpenWonton 0.8 uses a new RPC mechanism to route node-specific APIs like
...
OpenWonton 0.8 has changed the organization of CLI commands to be based on
...
as the old style will be deprecated in future versions of OpenWonton.
...
When no `advertise` address was specified and OpenWonton's `bind_addr` was loopback
or `0.0.0.0`, OpenWonton attempted to resolve the local hostname to use as an
advertise address.

Many hosts cannot properly resolve their hostname, so OpenWonton 0.6 defaults
...
OpenWonton 0.5.5 has a backward incompatible change in the `docker` driver's
...
OpenWonton 0.4.0 has backward incompatible changes in the logic for Consul
deregistration. When a Task which was started by OpenWonton v0.3.x is uncleanly shut
down, the OpenWonton 0.4 Client will no longer clean up any stale services. If an
in-place upgrade of the OpenWonton client to 0.4 prevents the Task from gracefully
shutting down and deregistering its Consul-registered services, the OpenWonton Client
...
OpenWonton 0.3.1 removes artifact downloading from driver configurations and places them as
a first class element of the task. As such, jobs will have to be rewritten in
the proper format and resubmitted to OpenWonton. OpenWonton clients will properly
re-attach to existing tasks but job definitions must be updated before they can
be dispatched to clients running 0.3.1.

## OpenWonton 0.3.0

OpenWonton 0.3.0 has made several substantial changes to job files included a new
...
be resubmitted with the updated job syntax using a OpenWonton 0.3.0 binary.
...
After updating the Servers and job files, OpenWonton Clients can be upgraded by first
...
deleted and then OpenWonton 0.3.0 can be launched.
...
[alloc_overlap]: https://github.com/openwonton/openwonton/issues/10440
...
[gh_10446]: https://github.com/openwonton/openwonton/pull/10446#issuecomment-1224833906
[gh_issue]: https://github.com/openwonton/openwonton/issues/new/choose
[gh-10875]: https://github.com/openwonton/openwonton/pull/10875
[gh-11563]: https://github.com/openwonton/openwonton/issues/11563
[gh-6787]: https://github.com/openwonton/openwonton/issues/6787
[gh-8457]: https://github.com/openwonton/openwonton/issues/8457
[gh-9148]: https://github.com/openwonton/openwonton/issues/9148
[go-client]: https://pkg.go.dev/github.com/openwonton/openwonton/api#Client
