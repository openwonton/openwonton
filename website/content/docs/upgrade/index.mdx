...
description: |-
  Learn how to upgrade OpenWonton.
---

# Upgrading

OpenWonton is designed to be flexible and resilient when upgrading from one OpenWonton
version to the next. Upgrades should cause neither a OpenWonton nor a service
outage. However, there are some restrictions to be aware of before upgrading:

- OpenWonton strives to be backward compatible for at least 1 point release, so
  OpenWonton v1.6.x hosts work with v1.5.x hosts. Upgrading 2 point releases
  (for example, from v1.4.0 to v1.6.0) may work but is untested and unsupported.

  - OpenWonton does _not_ support downgrading at this time. Downgrading clients
...
- When upgrading a OpenWonton Client, if it takes longer than the
  [`heartbeat_grace`][heartbeat_grace] (10s by default) period to restart, all
  allocations on that node may be rescheduled.

OpenWonton supports upgrading in place or by rolling in new servers:

- In Place: The OpenWonton binary can be updated on existing hosts. Running
  allocations will continue running uninterrupted.

- Rolling: New hosts containing the new OpenWonton version may be added followed by
...
binary on each host and restarting the OpenWonton service.

At a high level we complete the following steps to upgrade OpenWonton:
...
While it is possible to upgrade OpenWonton client nodes before servers, this guide
...
Whether you are replacing OpenWonton in place on existing systems or bringing up new
servers you should make changes incrementally, verifying cluster health at each
step of the upgrade.

On a single server, install the new version of OpenWonton. You can do this by
joining a new server to the cluster or by replacing or upgrading the binary
locally and restarting the OpenWonton service.
...
[Monitor the OpenWonton logs][monitor] on the remaining servers to check that the
new server has joined the cluster correctly.

Run `wonton agent-info` on the new servers and check that the `last_log_index`
is of a similar value to the other servers. This step ensures that changes have
been replicated to the new server.

```shell-session
ubuntu@nomad-server-10-1-1-4:~$ wonton agent-info
openwonton
  bootstrap = false
  known_regions = 1
  leader = false
...
Continue with the upgrades across the servers making sure to do a single OpenWonton
server at a time. You can check state of the servers with [`wonton server members`][server-members], and the state of the client nodes with [`wonton node status`][node-status].
...
Monitor the logs of the other hosts in the OpenWonton cluster over this period.
...
You are now running the latest OpenWonton version. You can verify all
...
between versions of open source OpenWonton. The same guidance above should be
...
protocol version 3 requires OpenWonton running 0.8.0 or newer on all
...
`node-id` instead of their IP address when OpenWonton makes changes to its
...
* If the upgrade is for a OpenWonton version lower than v1.3.0, update the
...
* On the server, run `wonton agent-info` and check that the
...
If you are running a single OpenWonton server, restarting it in-place will
...
NOMAD_DATA_DIR=$(wonton agent-info -json | jq -r '.config.DataDir')
NOMAD_ADDR=$(wonton agent-info -json | jq -r '.stats.nomad.leader_addr')
...
After running this script, if the upgrade is for a OpenWonton version lower
