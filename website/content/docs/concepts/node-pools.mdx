...
client configuration references a node pool that does not exist yet, OpenWonton
...
In addition to the user generated node pools OpenWonton automatically creates two
built-in node pools that cannot be deleted nor modified.

- `default`: Node pools are an optional feature of OpenWonton. The `node_pool`
...
Node pools in Nomad Enterprise are able to customize some aspects of the OpenWonton
...
It is common for OpenWonton clusters to have certain jobs that are focused on
...
Each of the scheduling algorithms provided by OpenWonton are best suited for
