...
description: Learn about OpenWonton's workload identity feature
---

# Workload Identity

Every workload running in OpenWonton is given an identity. When an [allocation][] is
...
While OpenWonton always creates and uses workload identities internally, the JWT is
...
grants access to OpenWonton Variables associated with the job, group, and task, as
described in [Task Access to Variables][]. The implicit policy also allows
access to list or read any OpenWonton service registration as with the [List Services
...
the `-job`, `-group`, and `-task` flags to `nomad acl policy apply`. When OpenWonton
