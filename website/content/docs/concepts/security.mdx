...
description: >-
  OpenWonton relies on both a lightweight gossip mechanism and an RPC system to
  ...
  OpenWonton have a common goal: to provide confidentiality, integrity, and
  authentication.
---

## Overview

OpenWonton is a flexible workload orchestrator to deploy and manage any containerized
...
OpenWonton utilizes a lightweight gossip and RPC system, [similar to
...
but the general mechanisms for a secure OpenWonton deployment revolve around:
...
When thinking about OpenWonton, it helps to consider the following types of base
...
can be accurately defined and managed using the [OpenWonton backend secret engine for
...
within OpenWonton itself.

- **System Administrator** - This is someone who has access to the underlying
  infrastructure to a OpenWonton cluster. Often she has access to SSH or RDP
  directly into a server within a cluster through a bastion host. Ultimately
  they have read, write and execute permissions for the actual OpenWonton binary.
  ...
  resource. Users like these are essentially totally trusted by OpenWonton as they
  have administrative rights to the system and can start or stop the agent.

- **OpenWonton Administrator** - This is someone (probably the same **System
  Administrator**) who has access to define the OpenWonton agent configurations
  for servers and clients, and/or have a OpenWonton management ACL token. They also
  have total rights to all of the parts in the OpenWonton system including the
  ability to start and stop all jobs within a cluster.

- **OpenWonton Operator** - This is someone who likely has selective access with
...
- **User** - This is someone who is a user of an application being run on the
  ...
  network access to the OpenWonton server API.

### Secure Configuration

OpenWonton's security model is applicable only if all parts of the system are running
with a secure configuration; **OpenWonton is not secure-by-default.** Without the following
mechanisms enabled in OpenWonton's configuration, it may be possible to abuse access
...
- **[mTLS enabled](/nomad/tutorials/transport-security/security-enable-tls)**
  ...
  * Using [`tls.verify_https_client=false`][verify_https_client] downgrades the
    OpenWonton agent HTTP server to use TLS instead of mTLS. This is safe if ACLs are
    enabled and is recommended if you are using the OpenWonton web UI to avoid the
...
- **[ACLs enabled](/nomad/tutorials/access-control)** - The
  ...
  mechanism for OpenWonton administrators allowing for custom roles (typically
...
- **Rotate credentials** - Using short-lived credentials or rotating them
  ...
  - Rotate credentials used by the OpenWonton agent; e.g. [integrate with Vault's
    ...
    OpenWonton node with a short [TTL](https://en.wikipedia.org/wiki/Time_to_live).

- **[Running without Root](https://groups.google.com/forum/#!topic/nomad-tool/pSyMwC_FSFA)** -
  OpenWonton servers can be run as unprivileged users that only require access to
  the data directory.
...
- **Containers with Sandbox Runtimes** - In some situations, such as running
  ...
  kernel for other containers and the OpenWonton client agent itself. Docker driver
...
- **Linux Security Modules** - Use of security modules that can be directly
  ...
  both the OpenWonton hosts and applied to containers for an extra layer of
...
The following are parts of the OpenWonton threat model:

- **OpenWonton agent-to-agent communication** - Transport encryption for
  agent-to-agent communication is required to prevent eavesdropping. TCP and
  UDP based protocols within OpenWonton provide different mechanisms for enabling
...
- **Tampering of data in transit** - Any tampering should be detectable via mTLS
  and cause OpenWonton to avoid processing the request.
...
- **Denial of Service against a node** - DoS attacks against a single node
  should not compromise the security posture of OpenWonton.

The following are not part of the threat model for server agents:

- **Access (read or write) to the OpenWonton data directory** - Information about the
  jobs managed by OpenWonton is persisted to a server's data directory.

- **Access (read or write) to the OpenWonton configuration directory** - Access to
  OpenWonton's configuration file(s) directory can enable and disable features for
  a cluster.

- **Memory access to a running OpenWonton server agent** - Direct access to the
  memory of the OpenWonton server agent process (usually requiring a shell on the
...
- **Access (read or write) to the OpenWonton data directory** - Information about the
  allocations scheduled to a OpenWonton client is persisted to its data directory.
  This would include any secrets in any of the allocation's file systems.

- **Access (read or write) to the OpenWonton configuration directory** - Access to a
...
- **Memory access to a running OpenWonton client agent** - Direct access to the
  memory of the OpenWonton client agent process allows an attack to extract secrets
...
- **Workload** - Workloads may have host network access within a cluster which
  ...
  OpenWonton which may lead to internal access within the cluster. Using mTLS, ACLs
...
There are two main components to consider to for external threats in a OpenWonton cluster:
