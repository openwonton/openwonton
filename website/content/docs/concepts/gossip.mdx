...
description: |-
  OpenWonton uses a gossip protocol to manage membership. All of this is provided
  through the use of the Serf library.
---

# Gossip Protocol

OpenWonton uses a [gossip protocol](https://en.wikipedia.org/wiki/Gossip_protocol)
...
## Gossip in OpenWonton

OpenWonton makes use of a single global WAN gossip pool that all servers participate in.
Membership information provided by the gossip pool allows servers to perform cross region
requests. The integrated failure detection allows OpenWonton to gracefully handle an entire region
...
this is not important, since the abstraction should be masked by OpenWonton. It can be useful
