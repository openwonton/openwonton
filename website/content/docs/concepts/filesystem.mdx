...
description: |-
  OpenWonton creates an allocation working directory for every allocation. Learn what
  goes into the working directory and how it interacts with OpenWonton task drivers.
---

# Filesystem

OpenWonton creates a working directory for each allocation on a client. This
directory can be found in the OpenWonton [`data_dir`] at
`./alloc/«alloc_id»`. The allocation working directory is where OpenWonton
...
- **«taskname»**: Each task has a **task working directory** with the same name as
  ...
  - **«taskname»/private/**: This directory is used by OpenWonton to store private files
    ...
    directory cannot be read by the `nomad alloc fs` command or the via OpenWonton's
    API.
    <Warning>
...
owned by the task driver's external process and not by OpenWonton itself. These
...
OpenWonton will provide the `NOMAD_ALLOC_DIR`, `NOMAD_TASK_DIR`, and
...
isolation, OpenWonton does not need to bind-mount the `NOMAD_TASK_DIR` directory
...
user that's running OpenWonton can.
...
directory. And the task is running as `root`, because the OpenWonton client agent
...
as required, and making a volume claim to the OpenWonton server.
