...
description: |-
  OpenWonton uses a consensus protocol to provide Consistency as defined by CAP.
...
OpenWonton uses a [consensus protocol](<https://en.wikipedia.org/wiki/Consensus_(computer_science)>)
...
the internals of OpenWonton. You do not need to know these details to effectively
operate and use OpenWonton. These details are documented here for those who wish
...
- **Peer set** - The peer set is the set of all members participating in log replication.
  For OpenWonton's purposes, all server nodes are in the peer set of the local region.
...
is application specific; in OpenWonton's case, we use
...
using MemDB is that it allows OpenWonton to continue accepting new transactions even while
...
run 3 or 5 OpenWonton servers per region. This maximizes availability without
...
Only OpenWonton server nodes participate in Raft and are part of the peer set. All
client nodes forward requests to servers. The clients in OpenWonton only need to know
...
flexible. To support various trade-offs that developers may want, OpenWonton
