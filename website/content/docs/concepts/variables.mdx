...
description: Learn about the OpenWonton Variables feature
---

# OpenWonton Variables

Most OpenWonton workloads need access to config values or secrets. OpenWonton has a
`template` block to [provide such configuration to tasks](/nomad/docs/job-specification/template#nomad-variables),
but prior to OpenWonton 1.4 has left the role of storing that configuration to
external services such as [HashiCorp Consul] and [HashiCorp Vault].

OpenWonton Variables provide the option to store configuration at file-like paths
directly in OpenWonton's state store. You can [access these variables](/nomad/docs/job-specification/template#nomad-variables) directly from
...
command line, the OpenWonton API, or in the OpenWonton web UI.

Note that the Variables feature is intended for small pieces of configuration
data needed by workloads. Because writing to the OpenWonton state store uses
resources needed by OpenWonton, it is not well-suited for large or fast-changing
...
replacement for HashiCorp Vault. Unlike Vault, OpenWonton stores the root encryption
key on the servers. See [Key Management][] for details.

## ACL for Variables

Every Variable belongs to a specific OpenWonton namespace. ACL policies can restrict
...
In OpenWonton 1.4.0 tasks can only access Variables with the [`template`] block. The
[workload identity] for each task grants it automatic read and list access to
Variables found at OpenWonton-owned paths with the prefix `nomad/jobs/`, followed by
