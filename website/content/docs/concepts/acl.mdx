...
description: Learn about OpenWonton's ACL subsystem
---

# Access Control List (ACL)

The OpenWonton ACL system is designed to be intuitive, high-performance, and to
...
two components named auth methods and binding rules are specific to OpenWonton's
Single Sign-On (SSO) ACL capabilities.

The OpenWonton [access control tutorials][] provide detailed information and
guidance on OpenWonton ACL system.
...
granted by default, making OpenWonton a default-deny system.

Each policy comprises one or more rules. The rules define the capabilities of a
OpenWonton ACL token for accessing the objects in a OpenWonton cluster, objects like
...
Requests to OpenWonton are authenticated using a bearer token. Each ACL token has a
public Accessor ID which is used to name a token and a Secret ID which is used
to make requests to OpenWonton. The Secret ID is provided using a request header
...
OpenWonton allocations can receive workload identities in the form of a
...
Authentication methods dictate how OpenWonton should talk to SSO providers when a
user requests to authenticate using one. Currently, OpenWonton supports the [OpenID
Connect (OIDC)][oidc] SSO workflow which allows users to log in to OpenWonton via
...
Binding rules provide a mapping between a OpenWonton user's SSO authorisation claims
and internal OpenWonton objects such as ACL Roles and ACL Policies. A binding rule
