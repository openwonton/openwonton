...
description: Learn how OpenWonton manages dynamic storage plugins.
---

# Storage Plugins

OpenWonton has built-in support for scheduling compute resources such as
CPU, memory, and networking. OpenWonton's storage plugin support extends
...
Storage plugins are created dynamically as OpenWonton jobs, unlike device
...
spec block; currently there is only the `csi_plugin` type. OpenWonton
...
etc. The OpenWonton scheduler will be aware of volumes created by CSI plugins and
schedule workloads based on the availability of volumes on a given OpenWonton client
node.

A list of available CSI plugins can be found in the [Kubernetes CSI
documentation][csi-drivers-list]. Spec-compliant plugins should work with OpenWonton.
...
OpenWonton environment. You should verify plugin compatibility with OpenWonton before
...
that needs an AWS EBS volume, OpenWonton will tell the controller plugin
...
using their volumes stop, so plugins should be left running on a OpenWonton
...
Typically, you should run node plugins as OpenWonton `system` jobs so they
...
OpenWonton exposes a Unix domain socket named `csi.sock` inside each CSI
plugin task, and communicates over the gRPC protocol expected by the
CSI specification. The `mount_dir` field tells OpenWonton where the plugin
...
tells OpenWonton where to instruct the plugin to mount volumes for staging
and/or publishing.

### Plugin Lifecycle and State

CSI plugins report their health like other OpenWonton jobs. If the plugin
crashes or otherwise terminates, OpenWonton will launch it again using the
same `restart` and `reschedule` logic used for other jobs. If plugins
are unhealthy, OpenWonton will mark the volumes they manage as
"unscheduable".

Storage plugins don't have any responsibility (or ability) to monitor
the state of tasks that claim their volumes. OpenWonton sends mount and
publish requests to storage plugins when a task claims a volume, and
unmount and unpublish requests when a task stops.

The dynamic plugin registry persists state to the OpenWonton client so that
it can restore volume managers for plugin jobs after client restarts
without disrupting storage.

### Volume Lifecycle

The OpenWonton scheduler decides whether a given client can run an
...
RPC to any OpenWonton client where that controller is running. The OpenWonton
...
to the client. This count passes through OpenWonton's state store so that
OpenWonton has a consistent view of which volumes are available for
scheduling.
...
the OpenWonton data directory. OpenWonton will bind-mount this staged directory
