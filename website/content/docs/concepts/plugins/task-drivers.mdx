...
description: Learn how to author a OpenWonton task driver plugin.
---

# Task Drivers

Task drivers in OpenWonton are the runtime components that execute workloads. For
a real world example of a OpenWonton task driver plugin implementation, see the [LXC
driver source][lxcdriver].

## Authoring Task Driver Plugins

Authoring a task driver (shortened to driver in this documentation) in OpenWonton
...
lifetime is not bound to the OpenWonton client. This means that the OpenWonton client can
be restarted without restarting the driver. OpenWonton will ensure that one
instance of the driver is running, meaning if the driver crashes or otherwise
terminates, OpenWonton will launch another instance of it.

Drivers should maintain as little state as possible. State for a task is stored
by the OpenWonton client on task creation. This enables a pattern where the driver
can maintain an in-memory state of the running tasks, and if necessary the
OpenWonton client can recover tasks into the driver state.
...
the OpenWonton client. This means the tasks lifecycle is distinct from the OpenWonton
client's.
...
1. The `TaskHandle` returned by `StartTask` will be propagated to replacement
   allocations if the OpenWonton client is drained or down. OpenWonton will call
   ...
   allocation.
2. If the OpenWonton client managing a remote task is drained or if the allocation
...
These behaviors are meant to keep remote tasks running even when the OpenWonton
...
OpenWonton handles all rotation and plumbing of task logs. In order for task stdout
and stderr to be received by OpenWonton, they must be written to the correct
location. Prior to starting the task through the driver, the OpenWonton client
...
compatible recovery of tasks. This field is opaque to OpenWonton, but allows the
...
disk. To support this, OpenWonton will attempt to recover a task that was
previously started if the driver does not recognize the task ID. During task
recovery, OpenWonton calls `RecoverTask` passing the `TaskHandle` that was
...
ID. If an error occurs, the OpenWonton client will mark the task as `lost`.
...
The OpenWonton client publishes events associated with an allocation. The
`TaskEvents` function allows the driver to publish driver specific events about
tasks and the OpenWonton client will associate them with the correct allocation.

An `Eventer` utility is available in the
`github.com/openwonton/openwonton/drivers/shared/eventer` package implements an
...
The `ExecTask` function is used by the OpenWonton client to execute commands inside
...
[lxcdriver]: https://github.com/openwonton/openwonton-driver-lxc
[driverplugin]: https://github.com/openwonton/openwonton/blob/v0.9.0/plugins/drivers/driver.go#L39-L57
[skeletonproject]: https://github.com/openwonton/openwonton-skeleton-driver-plugin
[baseplugin]: /nomad/docs/concepts/plugins/base
[taskconfig]: https://godoc.org/github.com/openwonton/openwonton/plugins/drivers#TaskConfig
[taskhandle]: https://godoc.org/github.com/openwonton/openwonton/plugins/drivers#TaskHandle
[fifopackage]: https://godoc.org/github.com/openwonton/openwonton/client/lib/fifo
