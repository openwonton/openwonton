...
description: Learn how to author a OpenWonton device plugin.
---

# Devices

OpenWonton has built-in support for scheduling compute resources such as CPU, memory,
and networking. OpenWonton device plugins are used to support scheduling tasks with
other devices, such as GPUs. They are responsible for fingerprinting these
devices and working with the OpenWonton client to make them available to assigned
tasks.

For a real world example of a OpenWonton device plugin implementation, see the [Nvidia
GPU plugin](https://github.com/openwonton/openwonton-device-nvidia).

## Authoring Device Plugins

Authoring a device plugin in OpenWonton consists of implementing the
...
A device plugin is long-lived. OpenWonton will ensure that one instance of the plugin is
running. If the plugin crashes or otherwise terminates, OpenWonton will launch another
instance of it.

However, unlike [task drivers](/nomad/docs/concepts/plugins/task-drivers), device plugins do not currently
have an interface for persisting state to the OpenWonton client. Instead, the device
...
It allows the plugin to provide OpenWonton with a list of discovered devices, along with their
...
[deviceplugin]: https://github.com/openwonton/openwonton/blob/v0.9.0/plugins/device/device.go#L20-L33
[baseplugin]: /nomad/docs/concepts/plugins/base
[skeletonproject]: https://github.com/openwonton/openwonton-skeleton-device-plugin
[fingerprintresponse]: https://github.com/openwonton/openwonton/blob/v0.9.0/plugins/device/device.go#L37-L43
[fingerprintfn]: https://github.com/openwonton/openwonton-skeleton-device-plugin/blob/v0.1.0/device/device.go#L159-L165
[statsfn]: https://github.com/openwonton/openwonton-skeleton-device-plugin/blob/v0.1.0/device/device.go#L169-L176
[reservefn]: https://github.com/openwonton/openwonton-skeleton-device-plugin/blob/v0.1.0/device/device.go#L189-L245
[dimensioned]: https://github.com/openwonton/openwonton/blob/v0.9.0/plugins/shared/structs/stats.go#L33-L34
