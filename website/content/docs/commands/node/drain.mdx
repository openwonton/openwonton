...
```plaintext
wonton node drain [options] <node>
```
...
```shell-session
$ wonton node drain -enable f4e8a9e5 -m "node maintenance"
...
```shell-session
$ wonton node drain -enable -ignore-system 4d2ba53b
...
```

Disable drain mode but keep the node ineligible for scheduling. Useful for
inspecting the current state of a misbehaving node without OpenWonton trying to
start or migrate allocations:

```shell-session
$ wonton node drain -disable -keep-ineligible 4d2ba53b
...
```
...
```shell-session
$ wonton node drain -enable -detach -self
...
$ wonton node drain -self -monitor
...
```
