...
```plaintext
wonton node status [options] [node]
```
...
information will be displayed. If running the command on a OpenWonton Client, the
...
```shell-session
$ wonton node status -short 1f3f03ea
ID        = c754da1f
Name      = nomad
Node Pool = default
Class     = <none>
...
Using `-self` when on a OpenWonton Client:

```shell-session
$ wonton node status -self
ID        = c754da1f
Name      = nomad-client01
Node Pool = default
Class     = <none>
...
```shell-session
$ wonton node status -stats c754da1f
ID        = c754da1f
Name      = nomad-client01
Node Pool = default
Class     = <none>
...
```shell-session
$ wonton node status -verbose c754da1f
