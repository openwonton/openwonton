...
```plaintext
wonton namespace apply [options] <input>
```
...
```shell-session
$ wonton namespace apply -description "Prod API servers" -quota prod api-prod
...
```shell-session
$ wonton namespace apply -quota= api-prod
```
...
$ wonton namespace apply namespace.hcl
```
