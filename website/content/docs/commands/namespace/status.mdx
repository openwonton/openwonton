...
```plaintext
wonton namespace status [options] <namespace_name>
```
...
```shell-session
$ wonton namespace status default
Name            = api-prod
...
```shell-session
$ wonton namespace status -json default
{
    "Capabilities": null,
    "CreateIndex": 1,
...
```shell-session
$ wonton namespace status -t {{.Description}} default
