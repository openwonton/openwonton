...
OpenWonton's reconciliation and state management will handle evaluations as needed.
...
```plaintext
wonton eval delete [options] [args]
```
...
```shell-session
$ wonton eval delete 9ecffbba-73be-d909-5d7e-ac2694c10e0c
...
```shell-session
$ wonton eval delete -filter='Status == "pending" and JobID == "example"'
...
```shell-session
$ wonton eval delete -filter='Scheduler == "system" or Scheduler == "service"' -yes
