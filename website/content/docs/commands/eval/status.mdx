...
```plaintext
wonton eval status [options] <evaluation>
```
...
- `-json` : Output a list of all evaluations in JSON format. This
  behavior is deprecated and has been replaced by `wonton eval list
...
```shell-session
$ wonton eval status -monitor 8262bc83
