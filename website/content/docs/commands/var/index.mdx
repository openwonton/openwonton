...
description: |-
  The "var" command groups subcommands for interacting with OpenWonton variables.
---

# Command: var

The `var` command is used to interact with OpenWonton [variables].

## Usage

Usage: `wonton var <subcommand> [options] [args]`

Run `wonton var <subcommand> -h` for help on that subcommand. The following
...
```shell-session
$ wonton var put -out=table secret/creds passcode=my-long-passcode
...
Update a value:
```shell-session
$ wonton var get secret/creds | nomad var put -in=json -out=table -v - user=dba
