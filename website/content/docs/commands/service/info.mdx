...
description: |
  The service info command is used to read the specified OpenWonton registered
  service by name.
---
...
```plaintext
wonton service info [options] <service_name>
```
...
```shell-session
$ wonton service info example-cache-redis
...
```shell-session
$ wonton service info -verbose example-cache-redis
