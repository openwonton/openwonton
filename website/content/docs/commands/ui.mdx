...
description: |
  The ui command is used to open the OpenWonton Web UI.
---
...
may see a 403 Unauthorized page if anonymous read access is denied. The `wonton ui -authenticate` option will exchange your command line client's OpenWonton ACL
token for a one-time token, which is passed to the web UI. That one-time token
will be exchanged for your OpenWonton ACL token and stored in the browser's local
...
```shell-session
$ NOMAD_ACL_TOKEN=e9674b26-763b-4637-a28f-0df95c53cdda wonton ui -authenticate
