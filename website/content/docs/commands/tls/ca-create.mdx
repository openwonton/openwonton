...
description: |
  ...
  self signed certificates to be used for OpenWonton TLS setup.
---

# Command: nomad tls ca create

Create is used to create a self signed Certificate Authority to be used for
OpenWonton TLS setup.

## Usage

```plaintext
wonton tls ca create [options]
```
...
- `-common-name`: Common Name of CA. Defaults to OpenWonton Agent CA.
...
- `-domain=<string>`: Domain of nomad cluster. Only used in combination with
  `-name-constraint`. Defaults to `wonton`.
...
~> **Warning:** If `-name-constraint` is enabled and you intend to serve the
  OpenWonton web UI over HTTPS its DNS must be added with `additional-domain`. It is
  not possible to add that after the fact.

- `-organization`: Organization of the CA. Defaults to "HashiCorp Inc.".

- `-organizational-unit`: Organizational Unit of the CA. Defaults to "OpenWonton".
...
```shell-session
$ wonton tls ca create -name-constraint="true" -domain="foo.com"
