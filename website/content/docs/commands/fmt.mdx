...
description: |
  Rewrite OpenWonton config and job files to canonical format
---

# Command: fmt

The `fmt` commands check the syntax and rewrites OpenWonton configuration and jobspec
files to canonical format. It can be used to improve readability and enforce
consistency of style in OpenWonton files.

## Usage

```plaintext
wonton fmt [flags] paths ...
```

Formats OpenWonton agent configuration and job file to a canonical format. If a path
