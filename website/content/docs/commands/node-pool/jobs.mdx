...
```plaintext
wonton node pool jobs [options] <node-pool>
```
...
```shell-session
$ wonton node pool jobs -namespace default prod
...
```shell-session
$ wonton node pool jobs -namespace '*' prod
...
```shell-session
$ wonton node pool jobs -per-page 2 prod
ID       Type     Priority  Status          Submit Date
job1     service  80        running         07/25/17 15:47:11 UTC
job2     batch    40        complete        07/24/17 19:22:11 UTC
...
wonton node pool jobs -per-page 2 -page-token job3 prod
