...
```plaintext
wonton node pool nodes [options] <node-pool>
```
...
```shell-session
$ wonton node pool nodes -filter='Datacenter == "dc2"' prod
ID        DC   Name      Class   Drain  Eligibility  Status
31c5347f  dc2  client-3  <none>  false  eligible     ready
6e98e378  dc2  client-2  <none>  false  eligible     ready
```
...
```shell-session
$ wonton node pool nodes -per-page=2 prod
ID        DC   Name         Class   Drain  Eligibility  Status
...
wonton node pool nodes -per-page=2 prod -page-token 6e98e378-b41a-86ac-8e5e-5ca8ab66236d
```
