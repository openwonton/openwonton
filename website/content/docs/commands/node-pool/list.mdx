...
```plaintext
wonton node pool list [options] <node-pool>
```
...
```shell-session
$ wonton node pool list -t '{{range .}}{{.Name}} [{{.Meta.env}}] - {{.Description}}{{println}}{{end}}'
...
```shell-session
$ wonton node pool info -t "{{.Name}} [{{.Meta.env}}] - {{.Description}}" prod
prod [production] - Node pool for production workloads.
```
...
```shell-session
$ wonton node pool list -per-page 2
Name     Description
all      Node pool with all nodes in the cluster.
default  Default node pool.
...
wonton node pool list -per-page 2 -page-token dev
