...
```plaintext
wonton quota inspect [options] <quota_name>
```
...
```shell-session
$ wonton quota inspect default-quota
{
    "Spec": {
        "CreateIndex": 8,
...
```shell-session
$ wonton quota inspect -json default-quota
...
```shell-session
$ wonton quota inspect -t {{ .Description }} default-quota
