...
description: |
  The agent command is the main entrypoint to running a OpenWonton client or server.
---

# Command: agent

The agent command is the heart of OpenWonton: it runs the agent that handles client
or server functionality, including exposing interfaces for client consumption
and running jobs.

Due to the power and flexibility of this command, the OpenWonton agent is documented
in its own section. See the [OpenWonton Agent] guide and the [Configuration]
...
A subset of the available OpenWonton agent configuration can optionally be passed in
...
- `-dev`: Start the agent in development mode. This enables a pre-configured
  ...
  OpenWonton. No other configuration is required to start the agent in this mode,
...
- `-retry-join`: Similar to `-join` but allows retrying a join if the first
  ...
  ```shell-session
  $ wonton agent -retry-join "127.0.0.1:4648"
  ```
...
[wonton agent]: /nomad/docs/operations/nomad-agent
