...
```plaintext
wonton alloc exec [options] <allocation> <command> [<args>...]
```
...
```shell-session
$ wonton alloc exec eb17e557 /bin/bash
...
$ wonton alloc exec eb17e557 cat /etc/resolv.conf
...
```
...
```shell-session
$ wonton alloc exec eb17e557 echo $NOMAD_ALLOC_ID # wrong
...
```
...
$ wonton alloc exec eb17e557 /bin/sh -c 'echo $NOMAD_ALLOC_ID'
eb17e557-443e-4c51-c049-5bba7a9850bc

$ # by using heredoc and passing command in stdin
$ wonton alloc exec eb17e557 /bin/sh <<'EOF'
> echo $NOMAD_ALLOC_ID
> EOF
eb17e557-443e-4c51-c049-5bba7a9850bc
```
...
$ wonton alloc exec eb17e557 /bin/sh -c 'cat /output | sort | uniq -c | sort -rn | head -n 5'
...
```
...
```plaintext
wonton alloc exec -job <job-id> <command> [<args>...]
```
...
$ wonton alloc exec -i -t -task mytask a1827f93 /bin/bash
a1827f93$
```
