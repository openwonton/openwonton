...
```plaintext
wonton alloc checks [options] <allocation>
```

Outputs the latest health check status information for services in the allocation
using the OpenWonton service discovery provider. This command accepts an allocation
...
```shell-session
wonton alloc checks -json 54fde4f8
...
```shell-session
$ wonton alloc checks -t '{{range .}}{{ printf "%s: %s\n" .ID  .Status }}{{end}}' 54
