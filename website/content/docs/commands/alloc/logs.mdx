...
```plaintext
wonton alloc logs [options] <allocation> <task>
```
...
Note that the `-no-color` option applies to OpenWonton's own output. If the task's
logs include terminal escape sequences for color codes, OpenWonton will not remove
them.

## Examples

```shell-session
$ wonton alloc logs eb17e557 redis
foobar
baz
bam

$ wonton alloc logs -stderr eb17e557 redis
[ERR]: foo
[ERR]: bar

$ wonton alloc logs -job example
[ERR]: foo
[ERR]: bar

$ wonton alloc logs -tail -n 2 eb17e557 redis
foobar
baz

$ wonton alloc logs -tail -f -n 3 eb17e557 redis
foobar
baz
bam
<blocking>
...
```shell-session
$ wonton alloc logs -task redis eb17e557
```
...
```shell-session
$ wonton alloc logs -task redis eb17e557 api
```
...
```plaintext
wonton alloc logs -job <job-id> <task>
```
