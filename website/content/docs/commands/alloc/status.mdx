...
failures, as well as the current state of its tasks. As of OpenWonton
0.7.1, alloc status also shows allocation modification time in
addition to create time. As of OpenWonton 0.8, alloc status shows
information about reschedule attempts. As of OpenWonton 0.11, alloc status
shows volume claims when a job claims volumes.

## Usage

```plaintext
wonton alloc status [options] <allocation>
```
...
```shell-session
$ wonton alloc status --short 0af996ed
ID                  = 0af996ed
Eval ID             = be9bde98
...
```shell-session
$ wonton alloc status -verbose 0af996ed
