...
```plaintext
wonton acl token create [options]
```
...
```shell-session
$ wonton acl token create -name="example-acl-token" -policy=example-acl-policy -role-name=example-acl-role
...
```shell-session
$ wonton acl token create -name="example-acl-token" -policy=example-acl-policy -ttl=8h
