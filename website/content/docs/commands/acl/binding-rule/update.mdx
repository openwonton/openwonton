...
```plaintext
wonton acl binding-rule update [options] <binding-rule_ID>
```
...
- `-bind-type`: Specifies adjusts how this binding rule is applied at login time
  to internal OpenWonton objects. Valid options are `role`, `policy`, and `management`.
...
```shell-session
$ wonton acl binding-rule update -description "new description" dc0e494d-0878-4e56-37d4-482e371e5bdb
