...
The `volume delete` command deletes external storage volumes with OpenWonton's
...
```plaintext
wonton volume delete [options] [volume]
```

The `volume delete` command requires a single argument, specifying the ID of
volume to be deleted. The volume must still be [registered] with OpenWonton in
