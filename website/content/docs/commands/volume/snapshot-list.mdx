...
```plaintext
wonton volume snapshot list [-plugin plugin_id -secrets key=value]
```
...
same as the OpenWonton volume ID, as the source volume may not be [registered] with
OpenWonton.
...
```shell-session
$ wonton volume snapshot list -plugin aws-ebs0
...
```shell-session
$ wonton volume snapshot list -plugin aws-ebs0 -secret key1=value1 -secret key2=val2
Snapshot ID  External ID  Size   Creation Time         Ready?
snap-12345   vol-abcdef   50GiB  2021-01-03T12:15:02Z  true
```
