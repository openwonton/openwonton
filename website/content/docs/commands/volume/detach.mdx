...
The `volume detach` command detaches external storage volumes with OpenWonton's
[Container Storage Interface (CSI)][csi] support.

## Usage

```plaintext
wonton volume detach [options] [volume] [node]
```
...
Note that you can use a node ID prefix just as you can with other OpenWonton
