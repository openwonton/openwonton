...
The `volume register` command registers external storage volumes with OpenWonton's
...
```plaintext
wonton volume register [options] [file]
```
...
file will be read and the job will be submitted to OpenWonton for scheduling. If the
