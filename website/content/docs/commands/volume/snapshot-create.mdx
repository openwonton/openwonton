...
```plaintext
wonton volume snapshot create [volume] [snapshot_name]
```
...
will be displayed. The volume must still be [registered] with OpenWonton in order
to be snapshotted.
...
```shell-session
$ wonton volume snapshot create ebs_prod_db1
...
```shell-session
$ wonton volume snapshot create ebs_prod_db1 snap-12345
Completed snapshot of volume ebs_prod_db1 with snapshot ID snap-12345.
```
