...
```plaintext
wonton volume status [options] [volume]
```
...
- `-verbose`: Show full information. Allocation create and modify times are
  ...
  cause OpenWonton to query the storage provider for volumes that are known to the
  storage provider but not yet registered with OpenWonton. This may include volumes
...
```shell-session
$ wonton volume [-type csi] status -verbose
ID            Name      Namespace  Plugin ID  Schedulable  Access Mode
ebs_prod_db1  database  default    ebs-prod   true         single-node-writer
...
```shell-session
$ wonton volume status [-verbose] [-plugin=ebs-prod] ebs_prod_db1
ID                   = ebs_prod_db1
Name                 = database
Type                 = csi
