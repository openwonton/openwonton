...
OpenWonton's [Container Storage Interface (CSI)][csi] support. The volume will be
removed from OpenWonton's state store but not deleted from the external storage
provider. Note that deregistering a volume prevents OpenWonton from deleting it via
[`volume delete`] at a later time.

## Usage

```plaintext
wonton volume deregister [options] [volume]
```
