...
```plaintext
wonton job dispatch [options] <parameterized job> [input source]
```
...
```shell-session
$ wonton job dispatch video-encode video-config.json
Dispatched Job ID = video-encode/dispatch-1485379325-cb38d00d
Evaluation ID     = 31199841
...
```shell-session
$ wonton job dispatch -detach video-encode video-config.json
...
```shell-session
$ wonton job dispatch -idempotency-token=prod video-encode video-config.json
Dispatched Job ID = video-encode/dispatch-1485379325-cb38d00d
Evaluation ID     = 31199841
...
```shell-session
$ wonton job dispatch -id-prefix-template=config1 video-encode video-config1.json
Jb
...
[parameterized job]: /nomad/docs/job-specification/parameterized 'OpenWonton parameterized Job Specification'
