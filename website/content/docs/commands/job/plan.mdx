...
downloaded and read from URL specified. OpenWonton downloads the job file using
...
submitting the job using [`wonton job run -check-index`], which will check that
...
- `-vault-token`: Used to validate if the user submitting the job has
  ...
  the OpenWonton server configuration. If the `-vault-token` flag is set, the passed
  Vault token is added to the jobspec before sending to the OpenWonton servers. This
...
- `-vault-namespace`: If set, the passed Vault namespace is stored in the job
  before sending to the OpenWonton servers.
...
```shell-session
$ wonton job plan example.nomad.hcl
+ Job: "example"
...
wonton job run -check-index 0 example.nomad.hcl
...
wonton job run -check-index 15 example.nomad.hcl
...
wonton job run -check-index 7 example.nomad.hcl
...
```shell-session
$ wonton job plan -verbose example.nomad.hcl
+/- Job: "example"
+/- Task Group: "cache" (3 create/destroy update)
...
When using the `wonton job plan` command in automated environments, such as
...
```console
$ wonton job plan -no-color example.nomad.hcl | tee /dev/stderr | grep 'Job Modify Index:' | awk -F': ' '{ print $2 }' > check-index || true
```
...
prevents undesired failures since `wonton job plan` returns a non-zero exit code
if a change is detected.

[job specification]: /nomad/docs/job-specification
[hcl job specification]: /nomad/docs/job-specification
[`go-getter`]: https://github.com/hashicorp/go-getter
[`wonton job run -check-index`]: /nomad/docs/commands/job/run#check-index
