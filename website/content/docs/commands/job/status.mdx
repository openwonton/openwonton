...
of the most useful status fields for each. As of OpenWonton 0.7.1, alloc status also
...
progress evaluation denotes that OpenWonton is blocked waiting for resources to
become available so that it can place the remaining allocations.

```shell-session
$ wonton job status -evals example
ID            = example
Name          = example
Submit Date   = 07/25/17 15:55:27 UTC
Type          = service
