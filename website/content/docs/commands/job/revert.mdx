...
Consul Connect enabled services and the OpenWonton servers were configured to require
...
persisted, it must be provided to revert if the targeted job version includes
Vault policies and the OpenWonton servers were configured to require [vault policy]
authentication.

## Usage

```plaintext
wonton job revert [options] <job> <version>
```
...
- `-consul-token`: If set, the passed Consul token is sent along with the revert
  request to the OpenWonton servers. This overrides the token found in the
