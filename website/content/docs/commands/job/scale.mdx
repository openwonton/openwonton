...
description: |
  The job scale command is used to change the count of a OpenWonton job group.
---

# Command: job scale

The `job scale` command is used to alter the number of running allocations within
a OpenWonton task group.

## Usage

```plaintext
wonton job scale [options] <job> <group> <count>
```
...
```shell-session
$ wonton job scale -detach job1 8
...
```shell-session
$ wonton job scale job1 group1 8
==> Monitoring evaluation "529cc88e"
    Evaluation triggered by job "job1"
    Evaluation within deployment: "28a3378f"
    Allocation "2a4df8ca" created: node "2f0a2f93", group "group1"
