...
read from URL specified. OpenWonton downloads the job file using [`go-getter`] and
...
```shell-session
$ wonton job validate -json example.json
...
```shell-session
$ wonton job validate example.nomad.hcl
Job Warnings:
1 warning(s):
