...
The `job run` command is used to submit new jobs to OpenWonton or to update existing
jobs. Job files must conform to the [job specification] format.

## Usage

```plaintext
wonton job run [options] <job file>
```
...
be submitted to OpenWonton for scheduling. If the supplied path is "-", the job file
...
- `-consul-token`: If set, the passed Consul token is stored in the job before
  sending to the OpenWonton servers. This allows passing the Consul token without
...
- `-consul-namespace`: <EnterpriseAlert inline/> If set, any services in the job will be registered into the
  ...
  [`consul` block `allow_unauthenticated`] is disabled in the OpenWonton server configuration, then
...
```shell-session
$ wonton job run example.nomad.hcl
...
```shell-session
$ wonton job run -check-index 5 example.nomad.hcl
Enforcing job modify index 5: job exists with conflicting job modify index: 6
Job not updated

$ wonton job run -check-index 6 example.nomad.hcl
...
```shell-session
$ wonton job run -detach example.nomad.hcl
...
```shell-session
$ wonton job run failing.nomad.hcl
