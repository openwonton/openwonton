...
stop the allocations and trigger the OpenWonton scheduler to compute new placements.
...
```plaintext
wonton job restart [options] <job>
```
...
```shell-session
$ wonton job restart -group=web example
...
```shell-session
$ wonton job restart -batch-size=2 example
...
```shell-session
$ wonton job restart -batch-size=50% example
...
```shell-session
$ wonton job restart -batch-size=2 -batch-wait=ask example
...
```shell-session
$ wonton job restart -batch-size=2 -batch-wait=10s example
