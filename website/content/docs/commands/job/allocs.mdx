...
```shell-session
$ wonton job allocs -verbose example
...
```shell-session
$ wonton job allocs -t '{{ range . }}{{ println .ID }}{{ end }}' example
...
```shell-session
$ wonton job allocs -t '{{ range . }}{{ if eq .ClientStatus "running" }}{{ println .ID }}{{ end }}{{ end }}' example
c2b4606d-1b02-0d8d-5fdd-031167cd4c91
```

Refer to the [Format OpenWonton Command Output With Templates][format_tutorial]
tutorial for more examples of using Go templates to format OpenWonton CLI output.
