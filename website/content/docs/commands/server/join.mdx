...
description: >
  The server join command is used to join the local server to one or more OpenWonton
  servers.
---

# Command: server join

The `server join` command joins the local server to one or more OpenWonton servers.
...
```plaintext
wonton server join [options] <addr> [<addr>...]
```
...
```shell-session
$ wonton server join 10.0.0.8:4648
