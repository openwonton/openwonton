...
description: >
  OpenWonton can be controlled via a command-line interface. This page documents
  all the commands OpenWonton accepts.
---

# OpenWonton Commands (CLI)

OpenWonton is controlled via a very easy to use command-line interface (CLI).
OpenWonton is only a single command-line application: `wonton`, which
takes a subcommand such as "agent" or "status". The complete list of
subcommands is in the navigation to the left.

The OpenWonton CLI is a well-behaved command line application. In erroneous cases,
a non-zero exit status will be returned. It also responds to `-h` and `--help`
as you would most likely expect.

To view a list of the available commands at any time, just run OpenWonton
...
OpenWonton's CLI supports command autocomplete. Autocomplete can be installed or
uninstalled by running the following on bash, zsh or fish shells:

```shell-session
$ wonton -autocomplete-install
$ wonton -autocomplete-uninstall
```

## Command Contexts

OpenWonton's CLI commands have implied contexts in their naming convention. Because
...
For example, the `wonton job run` command is used to run a new job, the `wonton status` command queries information about existing jobs, etc. Conversely,
commands with a prefix in their name likely operate in a different context.
Examples include the `wonton agent-info` or `wonton node drain` commands,
which operate in the agent or node contexts respectively.

### Remote Usage

The OpenWonton CLI may be used to interact with a remote OpenWonton cluster, even when the
local machine does not have a running OpenWonton agent. To do so, set the
`NOMAD_ADDR` environment variable or use the `-address=<addr>` flag when running
commands.

```shell-session
$ NOMAD_ADDR=https://remote-address:4646 wonton status
$ wonton status -address=https://remote-address:4646
```
...
OpenWonton can use environment variables to configure command-line tool options.
These environment variables can be overridden as needed using individual
flags.

#### Connection Environment Variables

- `NOMAD_ADDR` - The address of the OpenWonton server. Defaults to
  `http://127.0.0.1:4646`.

- `NOMAD_REGION` - The region of the OpenWonton server to forward commands to.
...
- `NOMAD_HTTP_AUTH` - (Optional) This allows users to supply "Basic" HTTP
  ...
  information in environments where the OpenWonton API is behind an authenticating
  proxy server.
...
- `NOMAD_CLIENT_CERT` - Path to a PEM encoded client certificate for TLS
  authentication to the OpenWonton server. Must also specify `NOMAD_CLIENT_KEY`.
...
- `NOMAD_CACERT` - Path to a PEM encoded CA cert file to use to verify the
  OpenWonton server SSL certificate.

- `NOMAD_CAPATH` - Path to a directory of PEM encoded CA cert files to verify
  the OpenWonton server SSL certificate. If both `NOMAD_CACERT` and `NOMAD_CAPATH`
