...
and exit non-0 if the license is not valid with the OpenWonton binary.

This command should always be run with the new binary before upgrading OpenWonton on a server,
as the server agent will not start with an invalid license.

## Usage

```plaintext
wonton license inspect [/path/to/license.hclic]
```
...
```shell-session
$ wonton license inspect /path/to/nomad/license.hclic
...
```shell-session
$ wonton license inspect license.hclic
...
Validation Errors:
