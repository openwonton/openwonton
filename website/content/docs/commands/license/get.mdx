...
license. The command is not forwarded to the OpenWonton leader, and will return
the license from the specific server being contacted.

~> License commands are new in OpenWonton 0.12.0 and are only available with OpenWonton
Enterprise.

## Usage

```plaintext
wonton license get [options]
```
