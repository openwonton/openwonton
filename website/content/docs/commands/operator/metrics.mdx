...
description: |-
  ...
  endpoint for metrics data related to the current OpenWonton
  process.
---
...
```plaintext
wonton operator metrics [options]
```
...
The output of `wonton operator metrics` is a JSON representation of the
[metrics API endpoint](/nomad/api-docs/metrics).

```shell-session
$ wonton operator metrics -pretty
{
  "Counters": [
    {
      "Count": 11,
