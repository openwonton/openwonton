...
description: |
  operator api is a utility command for accessing OpenWonton's HTTP API similar to
  the popular open source program curl.
---

# Command: operator api

The `operator api` command allows easy access to OpenWonton's HTTP API similar to
the popular [curl] program. OpenWonton's `operator api` command reads [environment
...
Accessing OpenWonton's [`/v1/jobs`][jobs] HTTP endpoint with `wonton operator
api` would require:

```
wonton operator api /v1/jobs
```
...
- `-dryrun`: output a curl command instead of performing the HTTP request
  ...
  for use in scripts or running in locations without a OpenWonton binary present.
...
```shell-session
$ wonton operator api -verbose /v1/agent/members?pretty
...
$ wonton operator api -region eu-west -filter 'Status == "completed"' -dryrun /v1/evaluations
curl \
  -X GET \
