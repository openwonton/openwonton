...
description: |
  The operator command provides cluster-level tools for OpenWonton operators.
---

# Command: operator

The `operator` command provides cluster-level tools for OpenWonton operators, such
as interacting with the Raft subsystem. This was added in OpenWonton 0.5.5.

~> Use this command with extreme caution, as improper use could lead to a OpenWonton
...
Usage: `wonton operator <subcommand> <subcommand> [options]`

Run `wonton operator <subcommand>` with no arguments for help on that subcommand.
...
- [`operator raft remove-peer`][remove] - Remove a OpenWonton server from the Raft
  configuration
...
- [`operator snapshot agent`][snapshot-agent] <EnterpriseAlert inline /> - Inspects a snapshot of the OpenWonton server state

- [`operator snapshot save`][snapshot-save] - Saves a snapshot of the OpenWonton server state

- [`operator snapshot restore`][snapshot-restore] - Restores a snapshot of the OpenWonton server state

- [`operator snapshot inspect`][snapshot-inspect] - Inspects a snapshot of the OpenWonton server state
