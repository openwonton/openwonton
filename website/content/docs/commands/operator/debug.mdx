...
The `operator debug` command builds an archive containing OpenWonton cluster
configuration and state information, OpenWonton server and client node
...
monitor logs for the `duration`, saving a snapshot of OpenWonton state
...
- `-interval=30s`: The interval between snapshots of the OpenWonton state.
...
- `-node-id=<node1>,<node2>`: Comma separated list of OpenWonton client node ids
...
- `-server-id=<server1>,<server2>`: Comma separated list of OpenWonton server names to
...
```shell-session
$ wonton operator debug -duration 5s -interval 5s -server-id all -node-id b5,20
Starting debugger...
...
```shell-session
$ wonton operator debug -duration 5s -interval 5s -server-id all -node-id all -max-nodes=1
Starting debugger...
