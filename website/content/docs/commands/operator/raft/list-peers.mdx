...
```plaintext
wonton operator raft list-peers [options]
```
...
- `Node` is the node name of the server, as known to OpenWonton, or "(unknown)" if
  the node is stale and not known.

- `ID` is the ID of the server. This is the same as the `Address` but may be
  upgraded to a GUID in a future version of OpenWonton.
...
- `Voter` is "true" or "false", indicating if the server has a vote in the Raft
  configuration. Future versions of OpenWonton may add support for non-voting servers.
