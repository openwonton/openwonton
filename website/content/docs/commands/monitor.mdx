...
description: |
  Stream the logs of a running OpenWonton agent.
---

# Command: monitor

The `wonton monitor` command streams log messages for a given agent.

## Usage

```plaintext
wonton monitor [options]
```

The nomad monitor command can be used to stream the logs of a
running OpenWonton agent. Monitor will follow logs and exit when
...
- `-server-id`: Specifies the nomad server id to stream logs from. Accepts
  server names from `wonton server members` and also a special `leader` option
...
```shell-session
$ wonton monitor -log-level=DEBUG -node-id=a57b2adb-1a30-2dda-8df0-25abb0881952
...
$ wonton monitor -log-level=DEBUG -json=true
