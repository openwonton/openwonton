...
description: Device Plugins are used to expose devices to tasks in OpenWonton.
---

# Device Plugins

Device plugins are used to detect and make devices available to tasks in
OpenWonton. Devices are physical hardware that exists on a client node such as a
GPU or an FPGA. By having extensible device plugins, OpenWonton has the flexibility
