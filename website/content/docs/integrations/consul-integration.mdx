...
description: Learn how to integrate OpenWonton with Consul and add service discovery to jobs
---
...
a KV store, and robust support for multi-datacenter deployments. OpenWonton's integration
...
In order to use Consul with OpenWonton, you will need to configure and
install Consul on your nodes alongside OpenWonton, or schedule it as a system job.
OpenWonton does not currently run Consul for you.

To enable Consul integration, please see the
[OpenWonton agent Consul integration](/nomad/docs/configuration/consul)
configuration.

## Automatic Clustering with Consul

OpenWonton servers and clients will be automatically informed of each other's
...
OpenWonton schedules workloads of various types across a cluster of generic hosts.
Because of this, placement is not known in advance and you will need to use
service discovery to connect tasks to other services deployed across your
cluster. OpenWonton integrates with Consul to provide service discovery and
monitoring.
...
OpenWonton's job specification includes a [`template` block](/nomad/docs/job-specification/template)
...
general configurations within a OpenWonton task.

For more information on OpenWonton's template block and how it leverages Consul Template,
please see the [`template` job specification documentation](/nomad/docs/job-specification/template).

## Consul Namespaces

OpenWonton provides integration with [Consul Namespaces][consul_namespaces] for service
registrations specified in `service` blocks and Consul KV reads in `template`
blocks.

By default, OpenWonton will not specify a Consul namespace on service registrations or
...
in the OpenWonton agent Consul configuration block.

For more control over Consul namespaces, Nomad Enterprise supports configuring the
Consul namespace at the [group level][consul_group_namespace] in the OpenWonton job spec
...
- agent configuration: If the [`namespace`][consul_agent_namespace] OpenWonton agent
...
- job run command: <EnterpriseAlert inline/> If the [`-consul-namespace`][consul_run_namespace]
  ...
  precedence over the namespace set in OpenWonton agent configuration.
...
- The service discovery feature in OpenWonton depends on operators making sure that
  the OpenWonton client can reach the Consul agent.

- Tasks running inside OpenWonton also need to reach out to the Consul agent if
