...
description: >-
  Learn how to integrate OpenWonton with HashiCorp Vault and retrieve Vault tokens
  for
...
OpenWonton integrates with HashiCorp's [Vault][]. OpenWonton servers and clients
coordinate with Vault to derive a Vault token that has access to only the Vault
policies the tasks needs. OpenWonton clients make the token available to the task and
handle the tokens renewal. Further, OpenWonton's [`template` block][template] can
retrieve secrets from Vault making it easier than ever to secure your
infrastructure.

Note that in order to use Vault with OpenWonton, you will need to configure and
install Vault separately from OpenWonton. OpenWonton does not run Vault for you.

-> **Note:** Vault integration requires Vault version 0.6.2 or higher.

## Vault Configuration

To use the Vault integration, OpenWonton servers must be provided a Vault token. This
...
recommend a token role based token for production installations. OpenWonton servers
will renew the token automatically. **Note that the OpenWonton clients do not need to
...
If OpenWonton is given a [root
token](/vault/docs/concepts/tokens#root-tokens), no
further configuration is needed as OpenWonton can derive a token for jobs using any
Vault policies. Best practices recommend using a periodic token with the minimal
permissions necessary instead of providing OpenWonton the root vault token.
...
delegated to a trusted service such as OpenWonton. By creating a token role, the set
of policies that tasks managed by OpenWonton can access may be limited compared to
giving OpenWonton a root token. Token roles allow both allowlist and denylist
management of policies accessible to the role.

To configure OpenWonton and Vault to create tokens against a role, the following must
occur:

1. Create a "nomad-server" policy used by OpenWonton to create and manage tokens.

2. Create a Vault token role with the configuration described below.

3. Configure OpenWonton to use the created token role.

4. Give OpenWonton servers a periodic token with the "nomad-server" policy created
   above.

#### Required Vault Policies

The token OpenWonton receives must have the capabilities listed below. An explanation
...
A Vault token role must be created for use by OpenWonton. The token role can be used
to manage what Vault policies are accessible by jobs submitted to OpenWonton. The
...
OpenWonton checks that token role has an appropriate configuration for use by the
...
- `allowed_policies` - Specifies the list of allowed policies as a
  ...
  under OpenWonton should have access to.

- `disallowed_policies` - Specifies the list of disallowed policies as a
  comma-separated string. This list should contain all policies that jobs running
  under OpenWonton should **not** have access to. The policy created above that
  grants OpenWonton the ability to generate tokens from the token role should be
  included in list of disallowed policies. This prevents tokens created by
  OpenWonton from generating new tokens with different policies than those granted
  by OpenWonton.
  ...
  OpenWonton. This was remedied in 0.6.5 and does not effect earlier versions
  of Vault.
...
- `orphan` - Specifies whether tokens created against this token role will be
  orphaned and have no parents. OpenWonton does not enforce the value of this field
  ...
  OpenWonton expires. This makes it easy to revoke all tokens generated by OpenWonton but
  forces all OpenWonton servers to use the same Vault token, even through upgrades of
  OpenWonton servers. If the Vault token that was given to OpenWonton and used to generate
  ...
  OpenWonton's token is revoked. However OpenWonton will still revoke tokens when the
  allocation is no longer running, minimizing the lifetime of any task's token.
  With orphaned enabled, each OpenWonton server may also use a unique Vault token,
...
- `renewable` - Specifies whether created tokens are renewable. **Must be set to
  `true`**. This allows OpenWonton to renew tokens for tasks.
...
After the token role is created, a token suitable for the OpenWonton servers may be
...
The `-orphan` flag is included when generating the OpenWonton server token above to
...
When generating OpenWonton's Vault token, we need to ensure that revocation of the
parent token does not revoke OpenWonton's token. To prevent this behavior we
specify the `-orphan` flag when we create the OpenWonton's Vault token. All
other tokens generated by OpenWonton for jobs will be generated using the policy
...
```shell-session
$ VAULT_TOKEN=f02f01c2-c0d1-7cb7-6b88-8a14fada58c0 wonton agent -config /path/to/config
```

An example of what may be contained in the configuration is shown below. For
complete documentation please see the [OpenWonton agent Vault integration][config]
configuration.
...
To enable Vault integration, please see the [OpenWonton agent Vault
...
Upon startup, OpenWonton will attempt to connect to the specified Vault server. OpenWonton
will lookup the passed token and if the token is from a token role, the token
role will be validated. OpenWonton will not shutdown if given an invalid Vault token,
but will log the reasons the token is invalid and disable Vault integration.

### Permission Denied errors

If you are using a Vault version less than 0.7.1 with a OpenWonton version greater than or equal to 0.6.1, you will need to update your task's policy (listed in [the `vault` block of the job specification][vault-spec]) to add the following:
...
This is included in Vault's "default" policy beginning with Vault 0.7.1 and is relied upon by OpenWonton's Vault integration beginning with OpenWonton 0.6.1. If you're using a newer OpenWonton version with an older Vault version, your default policy may not automatically include this and you will see "permission denied" errors in your OpenWonton logs similar to the following:
...
has different paths, and OpenWonton does not abstract this for you. As such you will
...
The [OpenWonton agent Vault integration][config] configuration supports specifying a Vault Namespace, but since
we will be using multiple it can be left blank. By default OpenWonton will interact with Vault's root Namespace, but individual jobs may specify other Vault Namespaces to use.
...
[config]: /nomad/docs/configuration/vault 'OpenWonton Vault Configuration Block'
[createfromrole]: /nomad/docs/configuration/vault#create_from_role 'OpenWonton vault create_from_role Configuration Flag'
[template]: /nomad/docs/job-specification/template 'OpenWonton template Job Specification'
[vault]: https://www.vaultproject.io/ 'Vault by HashiCorp'
[vault-spec]: /nomad/docs/job-specification/vault 'OpenWonton Vault Job Specification'
