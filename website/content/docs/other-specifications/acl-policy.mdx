...
description: Learn about OpenWonton's ACL policy specification.
---

# ACL Policy Specification

The [Secure OpenWonton with Access Control] guide includes step-by-step instructions
for bootstrapping OpenWonton's Access Control List (ACL) system, authoring policies,
...
Namespace rules control access to APIs in OpenWonton that are namespaced:
...
The OpenWonton web UI uses the API endpoints `/v1/agent` and `/v1/node` for nearly
every page. ACL policies for users who will use the OpenWonton UI should include the
following rules.
...
[Secure OpenWonton with Access Control]: /nomad/tutorials/access-control
