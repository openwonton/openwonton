...
- `limit_query` `(int: 20)` - Specifies the maximum number of OpenWonton objects to
  search through per context type in the OpenWonton server before truncating results.
  Setting this parameter to a high value may degrade OpenWonton server performance.
...
- `min_term_length` `(int: 2)` - Specifies the minimum size of the search term
  ...
  prevent unnecessary load on the OpenWonton server from broad queries.
