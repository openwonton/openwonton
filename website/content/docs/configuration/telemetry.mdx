...
description: |-
  The "telemetry" block configures OpenWonton's publication of metrics and telemetry
...
- `collection_interval` `(duration: 1s)` - Specifies the time interval at which
  the OpenWonton agent collects telemetry data.
...
- `publish_allocation_metrics` `(bool: false)` - Specifies if OpenWonton should
  publish runtime metrics of allocations.

- `publish_node_metrics` `(bool: false)` - Specifies if OpenWonton should publish
...
- `disable_dispatched_job_summary_metrics` `(bool: false)` - Specifies if OpenWonton
