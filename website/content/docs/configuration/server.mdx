...
description: |-
  The "server" block configures the OpenWonton agent to operate in server mode to
...
- `encrypt` `(string: "")` - Specifies the secret key to use for encryption of
  OpenWonton server's gossip network traffic. This key must be 32 bytes that are
  ...
  that to encrypt OpenWonton server's gossip protocol, this option only needs to be
  provided once on each agent's initial startup sequence. If it is provided
  after OpenWonton has been initialized with an encryption key, then the provided key
...
- `job_gc_interval` `(string: "5m")` - Specifies the interval between the job
  ...
  was introduced in OpenWonton 0.10.0.
...
- `default_scheduler_config` <code>([scheduler_configuration][update-scheduler-config]:
  ...
  `default_scheduler_config` was introduced in OpenWonton 0.10.4.
...
- `plan_rejection_tracker` <code>([PlanRejectionTracker](#plan_rejection_tracker-parameters))</code> -
  Configuration for the plan rejection tracker that the OpenWonton leader uses to
...
- `raft_protocol` `(int: 3)` - Specifies the Raft protocol version to use when
  communicating with other OpenWonton servers. This affects available Autopilot
  ...
  OpenWonton v1.4 or later.

- `raft_multiplier` `(int: 1)` - An integer multiplier used by OpenWonton servers to
  ...
  Tuning this affects the time it takes OpenWonton to detect leader failures and to
  ...
  By default, OpenWonton will use the highest-performance timing, currently equivalent
  ...
  leader elections pause OpenWonton's normal work, it may be beneficial for slow or
  ...
  (server crash) where a leader is lost, OpenWonton will not elect a new leader for
...
- `rejoin_after_leave` `(bool: false)` - Specifies if OpenWonton will ignore a
  previous leave and attempt to rejoin the cluster when starting. By default,
  OpenWonton treats leave as a permanent intent and does not attempt to join the
...
- `server_join` <code>([server_join][server-join]: nil)</code> - Specifies
  how the OpenWonton server will connect to other OpenWonton servers. The `retry_join`
...
- `upgrade_version` `(string: "")` - A custom version of the format X.Y.Z to use
  in place of the OpenWonton version when custom upgrades are enabled in Autopilot.
...
- `start_join` `(array<string>: [])` - Specifies a list of server addresses to
  join on startup. If OpenWonton is unable to join with any of the specified
...
unexpected issue. Refer to [Monitoring OpenWonton][monitoring_nomad_progress] for
more details.
...
This example shows a common OpenWonton agent `server` configuration block. The two
IP addresses could also be DNS, and should point to the other OpenWonton servers in
the cluster
...
The OpenWonton servers can automatically bootstrap if Consul is configured. For a
more detailed explanation, please see the
[automatic OpenWonton bootstrapping documentation](/nomad/tutorials/manage-clusters/clustering).
...
OpenWonton Clients periodically heartbeat to OpenWonton Servers to confirm they are
operating as expected. OpenWonton Clients which do not heartbeat in the specified
...
While OpenWonton Clients can connect to any Server, all heartbeats are forwarded to
the leader for processing. Since this heartbeat processing consumes resources,
OpenWonton adjusts the rate at which Clients heartbeat based on cluster size. The
...
minutes before OpenWonton recognizes that they are `down` and reschedules their
work.

[encryption]: /nomad/tutorials/transport-security/security-gossip-encryption 'OpenWonton Encryption Overview'
