...
description: Learn about the configuration options available for the OpenWonton agent.
---

# OpenWonton Configuration

OpenWonton agents have a variety of parameters that can be specified via
configuration files or command-line flags. Configuration files are written in
[HCL][hcl]. OpenWonton can read and combine parameters from multiple configuration
files or directories to configure the OpenWonton agent.

## Load Order and Merging

The OpenWonton agent supports multiple configuration files, which can be provided
...
```shell-session
$ wonton agent -config=server.conf -config=/etc/nomad -config=extra.json
```
...
Here is an example OpenWonton agent configuration that runs in both client and server
mode.
...
- `advertise` `(Advertise: see below)` - Specifies the advertise address for
  ...
  - `http` - The address to advertise for the HTTP interface. This should be
    reachable by all the nodes from which end users are going to use the OpenWonton
    CLI tools.

  - `rpc` - The address used to advertise to OpenWonton clients for connecting to OpenWonton
    servers for RPC. This allows OpenWonton clients to connect to OpenWonton servers from
    behind a NAT gateway. This address much be reachable by all OpenWonton client nodes.
    When set, the OpenWonton servers will use the `advertise.serf` address for RPC
    connections amongst themselves. Setting this value on a OpenWonton client has no
    effect.
  ...
  - `serf` - The address advertised for the gossip layer. This address must be
    ...
    this address. OpenWonton servers will communicate to each other over RPC using
    the advertised Serf IP and advertised RPC Port.

- `audit` `(`[`Audit`]`: nil)` - Enterprise-only. Specifies audit logging
  configuration.

- `bind_addr` `(string: "0.0.0.0")` - Specifies which address the OpenWonton
...
- `disable_anonymous_signature` `(bool: false)` - Specifies if OpenWonton should
  provide an anonymous signature for de-duplication with the update check.

- `disable_update_check` `(bool: false)` - Specifies if OpenWonton should not check
...
- `enable_debug` `(bool: false)` - Specifies if the debugging HTTP endpoints
  ...
  diagnostic information about OpenWonton's internals.
...
- `enable_syslog` `(bool: false)` - Specifies if the agent should log to syslog.
  ...
  the OpenWonton agent log set in `log_level`
...
- `limits` - Available in OpenWonton 0.10.3 and later, this is a nested object that
  ...
  - `rpc_handshake_timeout` `(string: "5s")` - Configures the limit for how
    ...
    negotiation. All OpenWonton clients should perform this immediately on
  ...
  - `rpc_max_conns_per_client` `(int: 100)` - Configures a limit of how
    ...
    and other server connections. OpenWonton clients multiplex many RPC calls over a
    ...
    endpoints; as of 0.10.3 OpenWonton client code does not. A reasonably low limit
...
- `log_level` `(string: "INFO")` - Specifies the verbosity of logs the OpenWonton
...
- `ports` `(Port: see below)` - Specifies the network ports used for different
  services required by the OpenWonton agent.
...
- `region` `(string: "global")` - Specifies the region the OpenWonton agent is a
...
You can send the OpenWonton process a `SIGHUP` signal to reload a limited subset of
...
- [`tls`][tls-reload]: note this only reloads the TLS configuration between
  OpenWonton agents (servers and clients), and not the TLS configuration for
  communication with Consul or Vault.
- [`vault`][vault-reload]: note this only reloads the TLS configuration
  between OpenWonton and Vault, but not other configuration values.

In order to reload any other configuration values, you must restart the OpenWonton
agent.
...
If the OpenWonton agent receives a `SIGHUP` during initialization, it may crash
(see [GH-3885]). Ensure that the OpenWonton agent is able to receive RPC traffic
...
This example shows configuring a custom region and data center for the OpenWonton
agent:
...
[`acl`]: /nomad/docs/configuration/acl 'OpenWonton Agent ACL Configuration'
[`audit`]: /nomad/docs/configuration/audit 'OpenWonton Agent Audit Logging Configuration'
[`client`]: /nomad/docs/configuration/client 'OpenWonton Agent client Configuration'
[`consul`]: /nomad/docs/configuration/consul 'OpenWonton Agent consul Configuration'
[`plugin`]: /nomad/docs/configuration/plugin 'OpenWonton Agent Plugin Configuration'
[`sentinel`]: /nomad/docs/configuration/sentinel 'OpenWonton Agent sentinel Configuration'
[`server`]: /nomad/docs/configuration/server 'OpenWonton Agent server Configuration'
[tls]: /nomad/docs/configuration/tls 'OpenWonton Agent tls Configuration'
[`vault`]: /nomad/docs/configuration/vault 'OpenWonton Agent vault Configuration'
...
[gh-3885]: https://github.com/openwonton/openwonton/issues/3885
