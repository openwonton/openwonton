...
description: >-
  The "audit" block configures the OpenWonton agent to configure Audit Logging
...
The `audit` block configures the OpenWonton agent to configure Audit logging behavior.
...
When enabled, each HTTP request made to an OpenWonton agent (client or server) will
...
- `path` `(string: "[data_dir]/audit/audit.log")` - Specifies the path and file
  name to use for the audit log. By default OpenWonton will use its configured
