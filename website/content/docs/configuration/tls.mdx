...
description: |-
  The "tls" block configures OpenWonton's TLS communication via HTTP and RPC to
...
The `tls` block configures OpenWonton's TLS communication via HTTP and RPC to enforce
...
authenticate each other. The OpenWonton documentation will typically use "TLS" to
...
the [Enable TLS Encryption for OpenWonton Tutorial](/nomad/tutorials/transport-security/security-enable-tls).

## `tls` Parameters

- `ca_file` `(string: "")` - Specifies the path to the CA certificate to use for
  OpenWonton's TLS communication.

- `cert_file` `(string: "")` - Specifies the path to the certificate file used
  for OpenWonton's TLS communication.
...
- `http` `(bool: false)` - Specifies if TLS should be enabled on the HTTP
  endpoints on the OpenWonton agent, including the API. By default this is non-mutual
  TLS. You can upgrade this to mTLS by setting
  [`verify_https_client=true`](#verify_https_client), but this can complicate
  using the OpenWonton UI by requiring mTLS in your browser or running a proxy in
  front of the OpenWonton UI.
...
- `rpc` `(bool: false)` - Toggle the option to enable mTLS on the RPC endpoints
  ...
  protection both between OpenWonton servers and from the clients back to the
  servers, ensuring mutual authentication. Setting `rpc=true` is required for
  secure operation of OpenWonton.
...
- `verify_https_client` `(bool: false)` - Specifies agents should require client
  ...
  the same CA as OpenWonton. By default, `verify_https_client` is set to `false`,
  which is safe so long as ACLs are enabled. This is recommended if you are
  using the OpenWonton web UI to avoid the difficulty of distributing client certs to
  browsers.
...
OpenWonton supports dynamically reloading both client and server TLS
configuration. To reload an agent's TLS configuration, first update the TLS
block in the agent's configuration file and then send the OpenWonton agent a `SIGHUP`
