...
description: |-
  The "vault" block configures OpenWonton's integration with HashiCorp's Vault.
  When configured, OpenWonton can create and distribute Vault tokens to tasks
  automatically.
---
...
The `vault` block configures OpenWonton's integration with [HashiCorp's
Vault][vault]. When configured, OpenWonton can create and distribute Vault tokens to
tasks automatically. For more information on the architecture and setup, please
see the [OpenWonton and Vault integration documentation][nomad-vault].
...
- `allow_unauthenticated` `(bool: true)` - Specifies if users submitting jobs to
  the OpenWonton server should be required to provide their own Vault token, proving
...
- `create_from_role` `(string: "")` - Specifies the role to create tokens from.
  The token given to OpenWonton does not have to be created from this role but must
  ...
  compatibility. It is recommended to set the `create_from_role` field if OpenWonton
...
- `token` `(string: "")` - Specifies the parent Vault token to use to derive
  child tokens for jobs requesting tokens. Only required on OpenWonton servers.
  OpenWonton client agents use the allocation's token when contacting Vault.
...
This example shows an example Vault configuration for a OpenWonton server:
...
This example shows an example Vault configuration for a OpenWonton client:
...
[nomad-vault]: /nomad/docs/integrations/vault-integration 'OpenWonton Vault Integration'
