...
description: |-
  The "ui" block configures the OpenWonton agent's web UI.
...
A default `ui` block is automatically merged with all OpenWonton agent
configurations. Note that the UI can be served from any OpenWonton agent,
...
- `consul` <code>([Consul]: nil)</code> - Configures integrations
  between the OpenWonton web UI and the Consul web UI.

- `vault` <code>([Vault]: nil)</code> - Configures integrations
  between the OpenWonton web UI and the Vault web UI.

- `label` <code>([Label]: nil)</code> - Configures a user-defined
  label to display in the OpenWonton Web UI header.
...
- `ui_url` `(string: "")` - Specifies the full base URL to a Consul
  ...
  is used to build links from the OpenWonton web UI to a Consul web
  ...
  by the OpenWonton to communicate with Consul, whereas the
...
- `ui_url` `(string: "")` - Specifies the full base URL to a Vault
  ...
  is used to build links from the OpenWonton web UI to a Vault web
  UI. Note that this URL will not typically be the same one used for
  the agent's [`vault.address`]; the `vault.address` is the URL used
  by the OpenWonton to communicate with Vault, whereas the
