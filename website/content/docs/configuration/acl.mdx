...
description: >-
  The "acl" block configures the OpenWonton agent to enable ACLs and tune various
  parameters.
---
...
The `acl` block configures the OpenWonton agent to enable ACLs and tunes various
ACL parameters. Learn more about configuring OpenWonton's ACL system in the [Secure
OpenWonton with Access Control guide][secure-guide].
...
- `enabled` `(bool: false)` - Specifies if ACL enforcement is enabled. All other
  ...
  same value for this parameter. For example the OpenWonton command line will
  send requests for client endpoints such as `alloc exec` directly to OpenWonton
...
- `token_min_expiration_ttl` `(string: "1m")` - Specifies the lowest acceptable
  TTL value for an ACL token when setting expiration. This is used by the OpenWonton
