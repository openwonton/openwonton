...
description: |-
  The "client" block configures the OpenWonton agent to accept jobs as assigned by
  the OpenWonton server, join the cluster, and specify driver-specific configuration.
---
...
- `network_interface` `(string: varied)` - Specifies the name of the interface
  ...
  If no non-local IP addresses are found, OpenWonton could fingerprint link-local IPv6
...
- `reserved` <code>([Reserved](#reserved-parameters): nil)</code> - Specifies
  that OpenWonton should reserve a portion of the node's resources from receiving
...
- `servers` `(array<string>: [])` - Specifies an array of addresses to the OpenWonton
...
- `server_join` <code>([server_join][server-join]: nil)</code> - Specifies
  how the OpenWonton client will connect to OpenWonton servers. The `start_join` field
...
- `gc_interval` `(string: "1m")` - Specifies the interval at which OpenWonton
...
- `gc_disk_usage_threshold` `(float: 80)` - Specifies the disk usage percent which
  OpenWonton tries to maintain by garbage collecting terminal allocations.

- `gc_inode_usage_threshold` `(float: 70)` - Specifies the inode usage percent
  which OpenWonton tries to maintain by garbage collecting terminal allocations.
...
- `no_host_uuid` `(bool: true)` - By default a random node UUID will be
  ...
  OpenWonton 0.6 the default was to use the system UUID.
...
- `bridge_network_name` `(string: "nomad")` - Sets the name of the bridge to be
  created by OpenWonton for allocations running with bridge networking mode on the
  client.
...
- `bridge_network_hairpin_mode` `(bool: false)` - Specifies if hairpin mode
  is enabled on the network bridge created by OpenWonton for allocations running
...
- `cgroup_parent` `(string: "/nomad")` - Specifies the cgroup parent for which cgroup
  subsystems managed by OpenWonton will be mounted under. Currently this only applies to the
...
see the [OpenWonton `exec` driver documentation](/nomad/docs/drivers/exec#chroot) for
the full list.

As of OpenWonton 1.2, OpenWonton will never attempt to embed the `alloc_dir` in the
chroot as doing so would cause infinite recursion.

### `options` Parameters

~> Note: In OpenWonton 0.9 client configuration options for drivers were deprecated.
See the [plugin block][plugin-block] documentation for more information.

The following is not an exhaustive list of options for only the OpenWonton
client. To find the options supported by each individual OpenWonton driver, please
...
- `"env.denylist"` `(string: see below)` - Specifies a comma-separated list of
  environment variable keys not to pass to these tasks. OpenWonton passes the host
...
- `set_environment_variables` `(string:"")` - Specifies a comma separated list
  ...
  the OpenWonton client's environment. By default a minimal environment is set including
...
- `wait` `(map: { min = "5s" max = "4m" })` - Defines the minimum and maximum amount
  ...
  templates whenever rendered variables from Consul, OpenWonton, or Vault change. However in
  order to minimize how often tasks are restarted or reloaded, OpenWonton will configure Consul
...
- `nomad_retry` `(map: { attempts = 12 backoff = "250ms" max_backoff = "1m" })` -
  This controls the retry behavior when an error is returned from OpenWonton. Consul
  Template is highly fault tolerant, meaning it does not exit in the face of failure.
  Instead, it uses exponential back-off and retry functions to wait for the cluster
  to become available, as is customary in distributed systems.
...
OpenWonton if the drain takes longer than allowed by the service shutdown. For
...
This example shows the most basic configuration for a OpenWonton client joined to a
cluster.
...
[metadata_constraint]: /nomad/docs/job-specification/constraint#user-specified-metadata 'OpenWonton User-Specified Metadata Constraint Example'
