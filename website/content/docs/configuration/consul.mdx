...
description: |-
  The "consul" block configures the OpenWonton agent's communication with
  Consul for service discovery and key-value integration. When
  configured, tasks can register themselves with Consul, and the OpenWonton cluster
...
configured, tasks can register themselves with Consul, and the OpenWonton cluster can
...
A default `consul` block is automatically merged with all OpenWonton agent
...
agent running on the same host as the OpenWonton agent with the default
configuration, OpenWonton will automatically connect and configure with Consul.

~> An important requirement is that each OpenWonton agent talks to a unique Consul
agent. OpenWonton agents should be configured to talk to Consul agents and not
Consul servers. If you are observing flapping services, you may have
multiple OpenWonton agents talking to the same Consul agent. As such avoid
configuring OpenWonton to talk to Consul via DNS such as consul.service.consul
...
- `allow_unauthenticated` `(bool: true)` - Specifies if users submitting jobs to
  the OpenWonton server should be required to provide their own Consul token, proving
...
- `auto_advertise` `(bool: true)` - Specifies if OpenWonton should advertise its
  services in Consul. The services are named according to `server_service_name`
  and `client_service_name`. OpenWonton servers and clients advertise their
  respective services, each tagged appropriately with either `http` or `rpc`
  tag. OpenWonton servers also advertise a `serf` tagged service.
...
- `client_auto_join` `(bool: true)` - Specifies if the OpenWonton clients should
...
- `client_service_name` `(string: "nomad-client")` - Specifies the name of the
  service in Consul for the OpenWonton clients.

- `client_http_check_name` `(string: "Nomad Client HTTP Check")` - Specifies the
  HTTP health check name in Consul for the OpenWonton clients.
...
- `server_service_name` `(string: "nomad")` - Specifies the name of the service
  in Consul for the OpenWonton servers.

- `server_http_check_name` `(string: "Nomad Server HTTP Check")` - Specifies the
  HTTP health check name in Consul for the OpenWonton servers.

- `server_serf_check_name` `(string: "Nomad Server Serf Check")` - Specifies
  the Serf health check name in Consul for the OpenWonton servers.

- `server_rpc_check_name` `(string: "Nomad Server RPC Check")` - Specifies
  the RPC health check name in Consul for the OpenWonton servers.

- `server_auto_join` `(bool: true)` - Specifies if the OpenWonton servers should
  automatically discover and join other OpenWonton servers by searching for the
...
- `share_ssl` `(bool: true)` - Specifies whether the OpenWonton client should share
...
- `tags` `(array<string>: [])` - Specifies optional Consul tags to be
  registered with the OpenWonton server and agent services.
...
If the local Consul agent is configured and accessible by the OpenWonton agents, the
OpenWonton cluster will [automatically bootstrap][bootstrap] provided
...
This example shows pointing the OpenWonton agent at a different Consul address. Note
...
permissions. OpenWonton requires `agent:read` permissions. In order to use the
`consul_namespace` feature, OpenWonton will need a token generated in Consul's
default namespace. That token should be created with `agent:read` as well
as a `namespace` block with the other relevant permissions for running OpenWonton
in the intended namespace. The Consul policy below shows an example policy
configuration for a OpenWonton server:
