...
description: >-
  The "server_join" block specifies how the OpenWonton agent will discover and
  connect to OpenWonton servers.
---
...
The `server_join` block specifies how the OpenWonton agent will discover and connect
to OpenWonton servers.
...
- `retry_join` `(array<string>: [])` - Specifies a list of server addresses to
  ...
  both OpenWonton servers and clients, while `start_join` is only defined for OpenWonton
...
- `start_join` `(array<string>: [])` - Specifies a list of server addresses to
  ...
  on the format of the string. This field is defined only for OpenWonton servers and
...
location of a OpenWonton server. There are many ways to reference a OpenWonton server,
including directly by IP address and resolving through DNS.

### Directly via IP Address

It is possible to address another OpenWonton server using its IP address. This is
...
It is possible to address another OpenWonton server using its DNS address. This is
...
As of OpenWonton 0.8.4, `retry_join` accepts a unified interface using the
[go-discover](https://github.com/hashicorp/go-discover) library for doing
