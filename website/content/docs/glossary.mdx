...
layout: docs
page_title: Glossary
description: Learn the definition of important OpenWonton concepts.
---

# Glossary

This glossary provides definitions and explanations for important terms and
concepts used in OpenWonton.
...
machines. Allocations are created by the OpenWonton servers as part of scheduling
...
the utilization of bins. This extends to OpenWonton, where the clients are "bins"
and the items are task groups. OpenWonton optimizes resources by efficiently bin
packing tasks onto client machines.

## Client

A OpenWonton client is an agent configured to run and manage tasks using available
...
OpenWonton agent is a long lived process which interfaces with the servers.

## Datacenters

OpenWonton models a datacenter as an abstract grouping of clients within a
region. OpenWonton clients are not required to be in the same datacenter as the
...
Deployments are the mechanism by which OpenWonton rolls out changes to cluster state
...
Evaluations are the mechanism by which OpenWonton makes scheduling decisions.  When
either the _desired state_ (jobs) or _actual state_ (clients) changes, OpenWonton
...
OpenWonton. A Job is a form of _desired state_; the user is expressing that the
job should be running, but not where it should be run. The responsibility of
OpenWonton is to make sure the _actual state_ matches the user desired state. A
Job is composed of one or more task groups.

## Node

A more generic term used to refer to machines running OpenWonton agents in client
...
OpenWonton models infrastructure as regions and datacenters. A region will contain
one or more datacenters. A set of servers joined together will represent a
single region. Servers federate across regions to make OpenWonton globally aware.
...
OpenWonton servers are the brains of the cluster. There is a cluster of servers per
...
A Task is the smallest unit of work in OpenWonton. Tasks are executed by drivers,
which allow OpenWonton to be flexible in the types of tasks it supports. Tasks
