...
description: |-
  Learn about how service discovery works in OpenWonton to connect workloads.
---
...
that can be used to query data. In OpenWonton there are two service catalogs:

- The **native service discovery** catalog is embedded in OpenWonton and requires no
...
parameter defining which catalog to use. OpenWonton stores the IP and port of all
...
OpenWonton native service discovery. The `template` can then be used as a
...
Both OpenWonton and Consul services can define health checks to make sure that only
