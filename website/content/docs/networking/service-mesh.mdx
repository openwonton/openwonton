...
description: |-
  Learn about how service mesh works in OpenWonton to securely connect and isolate
  your workloads.
---
...
OpenWonton has native integration with Consul to provide service mesh capabilities.
The [`connect`][] block is the entrypoint for all service mesh configuration.
OpenWonton automatically deploys a sidecar proxy task to all allocations that have a
...
Consul Service Mesh uses [Envoy][] as proxy. OpenWonton calls Consul's [`consul
connect envoy -bootstrap`][consul_cli_envoy] CLI command to generate the
initial proxy configuration.

OpenWonton injects a prestart sidecar Docker task to run the Envoy proxy. This task
...
for mesh-wide connectivity. OpenWonton can deploy these gateways using the
[`gateway`][] block. OpenWonton injects an Envoy proxy task to any `group` with a
`gateway` service.
...
- [OpenWonton Consul Service Mesh integration](/nomad/docs/integrations/consul-connect)
