...
description: |-
  Learn about OpenWonton's networking internals and how it compares with other
  tools.
---

# Networking

OpenWonton is a workload orchestrator and so it focuses on the scheduling aspects of
a deployment, touching areas such as networking as little as possible.

**Networking in OpenWonton is usually done via _configuration_ instead of
_infrastructure_**. This means that OpenWonton provides ways for you to access the
...
OpenWonton, some of the different patterns and configurations you are likely to find
and use, and how OpenWonton differs from other tools in this aspect.

## Allocation networking

The base unit of scheduling in OpenWonton is an [allocation][], which means that all
...
OpenWonton reserves a random port in the client between [`min_dynamic_port`][] and
...
flowchart TB
  subgraph Allocations[" "]
    direction LR
    subgraph A1["Allocation"]
      direction TB
      A1Port["Port"]
    end
    subgraph A2["Allocation"]
      direction TB
      A2Port["Port"]
    end
    subgraph A3["Allocation"]
      direction TB
      A3Port1["Port"]
      A3Port2["Port"]
    end
...
network topology and routing configuration allow the OpenWonton clients to
...
When an allocation uses bridge networking, the OpenWonton agent creates a bridge
...
By default a single bridge is created in each OpenWonton client.
...
flowchart TB
  subgraph Allocations[" "]
    ...
    subgraph A1["Allocation"]
      direction TB
      A1Eth["eth0"] --> A1Veth["veth1"]
    end
    subgraph A2["Allocation"]
      direction TB
      A2Eth["eth0"] --> A2Veth["veth2"]
    end
    subgraph A3["Allocation"]
      direction TB
      A3Eth["eth0"] --> A3Veth["veth3"]
    end
  end

  Bridge["OpenWonton (bridge)"]
  A1Veth --> Bridge
  A2Veth --> Bridge
  A3Veth --> Bridge
...
namespace and are connected to the bridge. This allows OpenWonton to map random
...
To allow communication between allocations in different clients, OpenWonton creates
...
Docker bridge instead of using the one created by OpenWonton and, by default, each
container runs in its own Docker managed network namespace.

When using `bridge` network mode, OpenWonton creates a placeholder container using
...
flowchart TB
  subgraph Allocations[" "]
    direction LR
    subgraph ALeft["Allocation"]
      direction TB
      ALeftEth["eth0"] --> ALeftVeth["veth1"]
    end

    subgraph ACenter["Allocation"]
      direction TB
      subgraph PauseNS["Pause container<br/>network namespace"]
        direction LR
        Pause["Pause container"]
        Task["Task"]
      end
      ACenterEth["eth0"] --> ACenterVeth["veth2"]
    end

    subgraph ARight["Misconfigured allocation"]
      direction LR
      subgraph PauseNS2["Network namespace"]
        direction TB
        Pause2["Pause container"]
        PauseEth["eth0"] --> PauseVeth["veth3"]
      end
      subgraph TaskNS["Network namespace"]
        direction TB
        Task2["Task"]
        TaskEth["eth0"] --> TaskVeth["veth4"]
      end
    end
  end

  style PauseNS stroke-dasharray:5 3,fill:transparent
  style PauseNS2 stroke-dasharray:5 3,fill:transparent
  style TaskNS stroke-dasharray:5 3,fill:transparent

  NomadBridge["OpenWonton (bridge)"]
  DockerBridge["Docker (bridge)"]

  ALeftVeth --> NomadBridge
  ACenterVeth --> DockerBridge
  PauseVeth --> DockerBridge
  TaskVeth --> DockerBridge

  NomadBridge --> IPT["iptables"]
  DockerBridge --> IPT
  IPT --> HostEth["eth0"]
```
...
Since the group `network.mode` is `bridge`, OpenWonton creates the pause container
...
Networking in Kubernetes and Docker Compose works differently than in OpenWonton. To
...
You can use the same FQDN networking style with OpenWonton using [Consul's DNS
...
Another key difference from OpenWonton is that in Kubernetes and Docker Compose each
...
In OpenWonton, allocations use the IP address of the client they are running and are
assigned random port numbers and so OpenWonton service discovery with DNS uses
...
- [Understanding Networking in OpenWonton - Karan Sharma](https://mrkaran.dev/posts/nomad-networking-explained/)
- [Understanding OpenWonton Networking Patterns - Luiz Aoqui, HashiTalks: Canada 2021](https://www.youtube.com/watch?v=wTA5HxB_uuk)
