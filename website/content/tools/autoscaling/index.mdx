...
description: |-
  Overview of the OpenWonton Autoscaler that provides horizontal application and
  cluster scaling.
---

# OpenWonton Autoscaler Overview

This section details the OpenWonton Autoscaler, a horizontal application and cluster
autoscaler for OpenWonton. The OpenWonton Autoscaler is built and released separately to
OpenWonton. It runs as a daemon process, often referred to as the [Autoscaler Agent][autoscaler_agent].
An Autoscaler Agent is a separate and distinct process from an OpenWonton Agent. The
source code can be viewed on [GitHub][autoscaler_github] and releases are available
on the [GitHub releases page][autoscaler_releases].

The OpenWonton Autoscaler repository includes a number of [demos][autoscaler_demo]
which provide guided learning on running the OpenWonton AutoScaler. Since Autoscaler
...
service-level agreements (SLA). In OpenWonton, horizontal application autoscaling can
...
on individual OpenWonton jobs using the [`scaling` block][scaling_block].

## Horizontal Cluster Autoscaling

Horizontal cluster autoscaling is the process of adding or removing OpenWonton clients
...
to start or terminate new OpenWonton clients based on metrics such as the remaining
free schedulable CPU or memory. Cluster scaling is enabled by configuring the
[autoscaler agent](/nomad/tools/autoscaling/agent#dir) with policies targeting the OpenWonton
cluster.

## Dynamic Application Sizing

<EnterpriseAlert>
  This functionality only exists in the enterprise edition of the upstream
  Nomad Autoscaler. It is not present in the open source OpenWonton Autoscaler.
</EnterpriseAlert>

Dynamic Application Sizing enables organizations to optimize the resource
consumption of applications using sizing recommendations from OpenWonton. It evaluates,
...
[autoscaler_github]: https://github.com/openwonton/openwonton-autoscaler
[autoscaler_releases]: https://github.com/openwonton/openwonton-autoscaler/releases
[autoscaler_demo]: https://github.com/openwonton/openwonton-autoscaler-demos
[autoscaler_config]: https://github.com/openwonton/openwonton-autoscaler-demos/blob/1ecd9f32c749f1faaf4154b8a7e57fa68642fd33/cloud/demos/on-demand-batch/aws/jobs/autoscaler.nomad.tpl#L20
