...
```mermaid
flowchart LR
  Setpoint["Setpoint"] --> Comparator((Comparator))
  Comparator --> Controller["Controller"]
  Controller --> Actuator["Actuator"]
  Actuator --> System["System"]
...
```mermaid
flowchart LR
  Policy["Policy"] --> Strategy((Strategy plugin))
  Strategy --> Autoscaler["Nomad Autoscaler"]
  Autoscaler --> TargetPlugin["Target plugin"]
  TargetPlugin --> Target["Target"]

  Target -->|Target status| TargetPlugin
  Target -->|Metrics| APM["APM plugin"]
  APM --> Strategy
  TargetPlugin --> Strategy
```
