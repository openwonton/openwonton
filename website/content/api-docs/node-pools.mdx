...
```shell-session
$ wonton operator api '/v1/node/pools'
```

```shell-session
$ wonton operator api '/v1/node/pools?prefix=prod'
```

```shell-session
$ wonton operator api -filter 'Meta.env == "production"' '/v1/node/pools?per_page=5'
```
...
```shell-session
$ wonton operator api /v1/node/pool/prod-eng
```
...
```shell-session
$ wonton operator api -X DELETE /v1/node/pool/prod-eng
```
...
```shell-session
$ wonton operator api /v1/node/pool/prod-eng/nodes
```

```shell-session
$ wonton operator api /v1/node/pool/prod-eng/nodes?os=true
```
...
```shell-session
$ wonton operator api /v1/node/pool/prod-eng/jobs
```

```shell-session
$ wonton operator api /v1/node/pool/prod-eng/jobs?namespace=prod
```
