...
This page will show a side bar navigation to traverse the job's task groups and tasks. The majority of this page is a latent terminal window. Using the side bar navigation will transition to the exec task page which establishes a `wonton exec` connection.
...
This page will show a side bar navigation to traverse the job's task groups and tasks with the selected task group already open. The majority of this page is a latent terminal window. Using the side bar navigation will transition to the exec task page which establishes a `wonton exec` connection.
...
This page streams log messages from a client's agent. It is the equivalent to the `wonton monitor` command with `-node-id` set to the client specified in the URL.
...
This page streams log messages from a server's agent. It is the equivalent to the `wonton monitor` command with `-server-id` set to the server specified in the URL.
