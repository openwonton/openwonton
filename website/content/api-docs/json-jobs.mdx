...
The [`wonton job run -output`][job-output] flag converts HCL jobs to JSON
without submitting the job:

```shell-session
$ wonton job run -output my-job.nomad
```

The [`wonton job inspect`][job-inspect] command retrieves the JSON specification
for an existing job:

```shell-session
$ wonton job inspect example
```

The [`wonton job run -json`][job-run-json] flag submits a JSON formatted job:

```shell-session
$ wonton job run -json example.json
```
...
$ wonton job run -output example.nomad.hcl
```
...
- `Periodic` - `Periodic` allows the job to be scheduled at fixed times, dates
  ...
  - `SpecType` - `SpecType` determines how OpenWonton is going to interpret the
...
- `Services` - `Services` is a list of `Service` objects. Nomad integrates with
  ...
  - `Provider`: Specifies the service registration provider to use for service
    registrations. Valid options are either `consul` or `wonton`. All services
...
can be queried by [external autoscalers](https://github.com/openwonton/openwonton-autoscaler).
...
- `Policy` - An optional JSON block. This is opaque to Nomad; see the documentation for the external
  autoscaler (e.g., [nomad-autoscaler](https://github.com/openwonton/openwonton-autoscaler)).
